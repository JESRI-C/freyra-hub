...
- **Tests:** Den transaktionelle pgTAP-kilde er `plan(107)` og dækker tenantmatricen, upload-intent, direct-insert-afvisning, idempotens, immutable binding, eksakt Storage-path, finalize/cancel, audit, legacy-read, udløb, medlemskabsrevoke, zoneproveniens samt service-role-only orphan-claims, leases, retry, stale-token, completion-ledger, orphan-row-retention og bevaret received-delete. De 107 cases er **ikke runtime-kørt her**. Cyklus 021: monitoring-uploadservicen har 35/35 tests for upload, exact-row/RLS-afledt download og delete; typecheck, målrettet ESLint/Prettier, fuld Vitest 57 filer/478 tests og normalt produktionsbuild PASS. Global lint er fortsat rød med 5.188 errors/23 warnings, men de to berørte filer har 0 fund. Én rigtig owner-bundet TUS/Storage/finalize/reload og en negativ fremmed-tenant-kontrol består; rigtig signed-download/expiry/revoke, resten af Storage-/rollematricen, lokal replay, pgTAP og DB-lint er **AFVENTER**.
- **Status:** delvist implementeret og anvendt på den isolerede staging. Den registrerede livehistorik indeholder `20260831172610`, `20260831172839`, `20260901163924`, `20260902153933`, `20260912110614` og `20260912132006`; sidste version retter finalizerens ugyldige `pg_catalog.coalesce(...)` uden at lempe caller-, owner-, projekt-, objekt- eller grantkontroller. Den samlede kildemigration `20260912112500` er fortsat ikke anvendt. Evidence- og monitoring-signed-download er nu implementeret i kilden med eksakt RLS-rækkeopslag, valideret DB-path og 300 s download-URL; monitoring accepterer kun rækkeafledt uploader-/intent-scope og fejler lukket på manipulerede paths eller Storage-fejl. Runtimeaccept af begge flows afventer deploy-/testmandat. Evidence size/MIME-kontrakt, Worker-secrets, automatisk cleanup-scheduling, delete/expiry/revoke, serverekstraktion og hele rollematricen mangler; P0 og produktion er fortsat **NO-GO**.
