...
- **Tests:** Den transaktionelle pgTAP-kilde er `plan(113)` og dækker tenantmatricen, upload-intent, direct-insert-afvisning, idempotens, immutable binding, eksakt Storage-path, finalize/cancel, audit, legacy-read, udløb, medlemskabsrevoke, zoneproveniens samt service-role-only orphan-claims, leases, retry, stale-token, completion-ledger, orphan-row-retention og bevaret received-delete. Seks nye assertions beviser, at tidligere contributors ikke kan probe en modtaget upload eller arkivere et pending intent, og at status/audit forbliver uændret. De 113 cases er **ikke runtime-kørt her**. Cyklus 022: målrettede migrationstests 2 filer/23 tests, typecheck, målrettet ESLint/Prettier, fuld Vitest 57 filer/479 tests og normalt produktionsbuild PASS. Global lint er fortsat rød med 5.188 errors/23 warnings, men de to berørte TypeScript-testfiler har 0 fund. Én rigtig owner-bundet TUS/Storage/finalize/reload og en negativ fremmed-tenant-kontrol består; rigtig signed-download/expiry/revoke, resten af Storage-/rollematricen, lokal replay, pgTAP og DB-lint er **AFVENTER**.
- **Status:** delvist implementeret og anvendt på den isolerede staging. Den registrerede livehistorik indeholder `20260831172610`, `20260831172839`, `20260901163924`, `20260902153933`, `20260912110614` og `20260912132006`; sidste liveversion retter finalizerens ugyldige `pg_catalog.coalesce(...)` uden at lempe caller-, owner-, projekt-, objekt- eller grantkontroller. Den samlede kildemigration `20260912112500` og den nye `20260913102814_enforce_upload_rpc_membership.sql` er fortsat ikke anvendt. Sidstnævnte kræver aktuelt projektbidrag før idempotent finalize-output eller cancel-mutation og bevarer låst `search_path` samt authenticated-only execute. Evidence- og monitoring-signed-download er implementeret i kilden med eksakt RLS-rækkeopslag, valideret DB-path og 300 s download-URL. Runtimeaccept af disse flows afventer deploy-/testmandat. Evidence size/MIME-kontrakt, Worker-secrets, automatisk cleanup-scheduling, delete/expiry/revoke, serverekstraktion og hele rollematricen mangler; P0 og produktion er fortsat **NO-GO**.
