...
- **Tests:** Den transaktionelle pgTAP-kilde er nu `plan(107)` og dækker tenantmatricen, upload-intent, direct-insert-afvisning, idempotens, immutable binding, eksakt Storage-path, finalize/cancel, audit, legacy-read, udløb, medlemskabsrevoke, zoneproveniens samt service-role-only orphan-claims, leases, retry, stale-token, completion-ledger, orphan-row-retention og bevaret received-delete. De 107 cases er **ikke runtime-kørt her**. Aktuel typecheck, målrettet ESLint, fuld serial Vitest 55 filer/421 tests og staging-build med ref-/bundle-preflight består. Frisk lokal replay, pgTAP, DB-lint og rigtig TUS/Storage list/read/write/delete/cleanup/`UPDATE`-afvisnings-/signed-URL-integration er **AFVENTER**.
- **Status:** delvist implementeret og anvendt på den isolerede staging. Den registrerede livehistorik stopper ved `20260831172610`, `20260831172839`, `20260901163924`, `20260902153933` og `20260912110614`; sidste version retter cleanup-claimets SQLSTATE `42702` uden at lempe grants eller RLS. Den samlede kildemigration `20260912112500` er ikke anvendt, fordi der ikke forelå særskilt live-migrationsmandat, selv om live schema allerede matcher den hærdede postflight. Uploadflowet kræver en idempotent, kortlivet server-intent og eksakt TUS-path uden upsert; orphan-reconciliation bruger private service-role-only claims og en persisteret eksakt lease-grænse. Evidence-kontrakten, Worker-secrets, automatisk cleanup-scheduling, rigtig Auth-/TUS-/Storage-verifikation og hele rollematricen mangler; P0 og produktion er fortsat **NO-GO**.
- **Evidens/commit:** Live postflight på `xdvqdzdpyceojbdknofi` viser private lease-ledger med RLS, ingen Data API-tabelprivilegier, gyldige indeks og kun `service_role`-execute på cleanup-RPC'erne. `public.uploads` og `storage.objects` havde 0 rækker; tomt cleanup-claim returnerede 0 rækker efter `42702`-rettelsen. En rollback-only `authenticated` canary bestod projektinsert, Polygon-persistence, server-scopet pending drone-intent og metadata og efterlod ingen canary-rækker. Security advisor viste forventet private/no-policy INFO samt kendte PostGIS-/`spatial_ref_sys`- og leaked-password-fund; performance advisor returnerede `47/21/76/18`. Rigtig Auth-/TUS-/Storage-/cleanup-accept, den fulde rollematrix og lokale replay-/pgTAP-/lint-gates mangler fortsat. Lovable-/produktionsinstansen `ikrmcetjutqcjtwfhzfv` er ikke ændret.
...
- **Tests:** Cyklus 008's fem browserklienttests dækker singleton-identitet, miljøfallback og browser-/SSR-options. Cyklus 010 tilføjer 9/9 regressionstests for deferred/latest authbootstrap, stale/cancelled jobs, identity-vs-token-refresh, hydreringsstyret loginredirect og afvisning af eksterne/backslash-redirects. Cyklus 017 tilføjer regression for atomisk cross-org projektvalg. Aktuel samlet suite er 55 filer/421 tests PASS. Credential-båret login/reset/logout/refresh/account-switch samt direkte reload på detalje-/geometri-/kortruterne er **AFVENTER**.
- **Status:** delvist implementeret; ikke produktion. `src/lib/supabase/client.ts` ejer den eneste browser-Supabase-/GoTrue-instans. `onAuthStateChange` foretager ikke længere async Supabase-kald inde i callbacken: bootstrap køres deferred, kun seneste identitet må committe tenantstate, same-user token-events opdaterer alene sessionen, og manuelle refreshes afmonterer ikke aktive workflows. Password-login venter på en hydreret bruger før deklarativ navigation; bootstrapfejl vises med retry, og `next` er begrænset til en sikker same-origin relativ sti. Projektvalg finder nu projektets organisation i de indlæste, autoriserede medlemskaber og opdaterer organisation/projekt atomisk; ukendte projekt-ID'er afvises. Projektdetalje, projektgeometri og projektkort har `ssr:false`; projektindekset har ingen beskyttet loader og ligger under auth-layoutet. Supabase Auth Site URL/redirect-allowlist, credential-båret browserrejse og direkte reload er fortsat **AFVENTER**; request-scoped serverklient er kun nødvendig, hvis beskyttede SSR-loaders genindføres.
- **Evidens/commit:** Authbootstrap-/loginhelpers og tests, `src/lib/auth.tsx` og `src/routes/login.tsx`; commit `fd48949` er pushet til `origin/codex/gofreyra-p0` og deployet på Worker-version `842dbf65-3e99-4fd1-83f5-62360268181d` i `gofreyra-p0-staging`. 14 målrettede auth-/selection-tests, typecheck, målrettet ESLint, fuld serial Vitest 55 filer/421 tests, build, Wrangler dry-run, deploy og anonym hosted smoke består; credential-båret cross-org navigation er **AFVENTER**.
...
- **Evidens/commit:** Cyklus 011: 3/3 workflowkontrakttests, typecheck, `verify:faktorer`, fuld serial Vitest 46 filer/362 tests, målrettet ESLint/Prettier, YAML parse og normal build består. Cyklus 017: typecheck, målrettet ESLint, fuld serial Vitest 55 filer/421 tests og staging-build med env-preflight/bundle-check består. Workflowkilden ligger i `.github/workflows/ci.yml`; hosted Linux-run, global lint og npm audit-triage er endnu ikke afsluttet.
...
- **Tests:** 8 filer og 68/68 historiske pure/service-tests dækker lukning, WGS84-range, unikke/nabopunkter, self-intersection, nulareal, Polygon-huller, areal/centroid, unsupported typer, importfejl, persistence/no-write/clear/concurrency, seed-after-clear, edit-state guards og canonical/fail-closed eksport. Den nye UI-gate stopper manglende eller malformed geometri. Rollback-canary bekræftede gyldig Polygon-persistence på staging; credential-båret browser import/edit/save/reload er `AFVENTER`.
- **Status:** delvist implementeret. Alle boundary-skriveveje bruger én fail-closed validator; persistent Polygon kan redigeres med eksplicit Gem/Annuller og frosset lag under save; ugemte edits blokerer konfliktende boundary-operationer; clear nulstiller alle afledte felter uden seed-genoplivning. Projektgrænsen fører nu til en projekt-scopet `FØR`-CTA. Detalje-, geometri- og kortruter har `ssr:false`; projektindekset ligger under auth-layoutet uden protected loader. Immutable revision, atomisk boundary+RPC-snapshot, metrics-friskhed mod boundary-version, cross-tab optimistic concurrency og credential-båret browserflow er `AFVENTER`.
...
- **Status:** delvist implementeret. Dronefotos routes gennem uploadvalidering; browserens hash, rå EXIF/XMP, normaliserede geodata og QA ligger tabsfrit som ubetroet `uploads.user_metadata.client_preview`, og syntetisk centroid bruges ikke. En betroet serverekstraktor skal udfylde `detected_metadata`. En projektbundet BEFORE-batch på op til 200 billeder med stabil rækkefølge, to samtidige jobs, server-intent, TUS-resume og retry-samme-intent er app-/servicetestet; intent-/cleanup-migrationerne er anvendt og rollback-canary-verificeret på staging. UI med synlig `Upload`-fane og projekt-scopet `FØR`-CTA er deployet på Worker-version `842dbf65-3e99-4fd1-83f5-62360268181d`, men Worker-secrets/scheduler og rigtig credential-båret browser/TUS/Storage/finalize/cleanup er **AFVENTER**. Det samme gælder UI-pause/reload-resume, survey rounds, fotopar, reviewerflow, canonical `drone_assets`, footprint/ortofoto og fuld tenantverifikation.
