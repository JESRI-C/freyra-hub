...
- **Tests:** Cyklus 008's fem browserklienttests dækker singleton-identitet, miljøfallback og browser-/SSR-options. Cyklus 010 tilføjer 9/9 regressionstests for deferred/latest authbootstrap, stale/cancelled jobs, identity-vs-token-refresh, hydreringsstyret loginredirect og afvisning af eksterne/backslash-redirects. Cyklus 017 tilføjer regression for atomisk cross-org projektvalg. Aktuel samlet suite er 55 filer/422 tests PASS. Credential-båret login/session, organisationsvalg, projekt create/open og reload af det testede projekt-/uploadflow består; reset/logout/refresh/account-switch og direkte reload af projektkortet er **AFVENTER**.
