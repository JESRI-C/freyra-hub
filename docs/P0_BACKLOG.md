...
- **Cyklus 015 staging-app-evidens:** App-pakken på commit `adcb03e` er udgivet til den isolerede Worker `gofreyra-p0-staging`, version `51381b65-7bfb-499d-ae4c-d3175827adcf`. Loginrendering, anonym redirect og tom browserkonsol består. Upload-intent-/orphan-migrationerne er ikke anvendt, og Workerens secretliste mangler `SUPABASE_SERVICE_ROLE_KEY` samt `MONITORING_CRON_API_SECRET`; reconciliation-routen returnerer derfor fail-closed HTTP 503. Dette ændrer ikke den fortsatte P0-/produktions-`NO-GO`.
...
- **Status:** delvist implementeret og UI-koden er udgivet på isoleret staging. Dronefotos routes gennem uploadvalidering; browserens hash, rå EXIF/XMP, normaliserede geodata og QA ligger tabsfrit som ubetroet `uploads.user_metadata.client_preview`, og syntetisk centroid bruges ikke. En betroet serverekstraktor skal udfylde `detected_metadata`. En projektbundet BEFORE-batch på op til 200 billeder med stabil rækkefølge, to samtidige jobs, server-intent, TUS-resume og retry-samme-intent i den åbne wizard er app-/servicetestet. Det nye intent-/cleanup-flow kan endnu ikke bruges end-to-end på staging, fordi de to forward migrationer og nødvendige server-secrets ikke er anvendt. UI-pause/reload-resume, survey rounds, fotopar, synkroniserede sammenligninger, reviewerflow, canonical `drone_assets`-routing, footprint/ortofoto og live tenant-/Storage-verifikation er **AFVENTER**.
