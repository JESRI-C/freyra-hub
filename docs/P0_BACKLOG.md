...
- **Cyklus 012 rigtig bruger-JWT-evidens:** Brugerens staging-login lykkedes og en rigtig `owner`-session nåede et `projects`-insert. Live log og rollback-beskyttet rolle/JWT-diagnose viste, at write-helperen tillod tenantens org, plain `INSERT` bestod, og kun `INSERT ... RETURNING` kolliderede med den eksisterende read-policys same-command snapshot. Appen opretter derfor med kendt UUID uden representation og foretager selection i en separat, frisk RLS-filtreret request; policies og tenantgrænser er uændrede. Unit/service/typecheck/build/dry-run er grønne; brugerens browser create/open/reload-smoke efter deploy og resten af rollematricen er fortsat `AFVENTER`.
