...
- Den isolerede staging `xdvqdzdpyceojbdknofi` er `ACTIVE_HEALTHY`. Den registrerede livehistorik stopper ved `20260831172610`, `20260831172839`, `20260901163924`, `20260902153933` og `20260912110614`; upload-intent, orphan-reconciliation og `42702`-korrektionen er dermed anvendt dér. Den samlede kildemigration `20260912112500` er ikke anvendt, fordi der ikke forelå et særskilt live-migrationsmandat, selv om live schema matcher den observerede hærdede postflight. Det samlede schema er ikke produktionsklart: frisk lokal replay, pgTAP `plan(107)`, DB-lint, rigtig Auth/Storage/TUS/cleanup-test og review af produktionsdata mangler fortsat.
...
Efter migration skal mindst to organisationer testes negativt for tabel- og Storage-adgang. Cyklus 009 har udført en transaktionel live SQL-test som `authenticated` med tenant A/B og rollback. Cyklus 016's rollback-only `authenticated` canary bestod projektinsert, Polygon-persistence samt server-scopet pending drone-intent/metadata og efterlod ingen rækker. Rigtig PostgREST bruger-JWT og Storage API list/read/write/delete/signed-URL/revoke mangler fortsat. Service-role-resultater er ikke RLS-evidens.

Security advisor viser fortsat de kendte PostGIS-/`spatial_ref_sys`-fund og leaked-password-beskyttelse samt forventet INFO for private RLS-tabeller uden policies. Performance advisor returnerede count-vector `47/21/76/18`; den er ikke triageret. `public.spatial_ref_sys` må ikke ændres alene eller automatisk, fordi det kan blokere legitim PostGIS-adgang. Miljøejer skal vælge grants og read-policy først.
...
- Projektbundet BEFORE-batch, serverudstedt upload-intent med eksakt pending-row-binding, 6 MiB TUS-chunks, retry-samme-intent og exact-path orphan-reconciliation er implementeret; databasekontrakten er anvendt og rollback-canary-verificeret på staging. Batch-UI'et har ingen egentlig pause- eller reload-resume-kontrol endnu. Canonical routing til `drone_assets`, evidence size/MIME-kontrakt, Worker-secrets, cleanup-scheduling samt rigtig browser-/Storage-/TUS-/cleanup-test er **AFVENTER**.
...
Kør desuden målrettede tests før hele suiten. Verificeret worktree 2026-09-12, cyklus 016: typecheck PASS; målrettet ESLint på ændrede filer PASS; fuld serial Vitest 55 filer/420 tests PASS; staging-build med env-preflight og bundle-check for `xdvqdzdpyceojbdknofi` PASS; Wrangler `deploy --env staging --keep-vars --strict --dry-run` PASS. Første buildforsøg ramte sandbox-read-denial; den godkendte genkørsel bestod og er den gældende evidens. Commit `57cdfe8` er pushet og hostet på staging som Worker-version `80e8ba0f-90b6-49f1-abce-3a8b1702af14`; loginrendering og anonyme redirects fra `/app` og direkte projektrute består. Migrationskilden har pgTAP `plan(107)`, men de 107 cases er ikke runtime-kørt; lokal replay/pgTAP/DB-lint forbliver blokeret uden lokal Postgres på `127.0.0.1:54322`. Cyklus 014's 54/412 er historisk, ikke den aktuelle testtælling. Seneste globale lint er fortsat rød med 5.155 errors/23 warnings og skal ned på 0 før P0-release.
...
- **Intent/TUS/finalize fejler:** Genbrug kun samme request-ID, fil og eksakte intent-path ved retry. En draft må ikke markeres modtaget uden vellykket server-finalize. Cancel arkiverer intentmetadata, men sletter ikke nødvendigvis et allerede overført objekt; behold intent-ID/path til smal, autoriseret reconciliation og forsøg aldrig en bred bucket-oprydning.
