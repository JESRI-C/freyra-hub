...
- `20260831064838_harden_4dm_tenant_isolation.sql` er anvendt på den isolerede staging som del af registreret baseline efter atomisk rollback-dry-run og live katalogassertions. Forward migrationerne `20260901163924_upload_intents_resumable_storage.sql` og `20260902153933_reconcile_upload_intent_orphans.sql` er kun i kilden og er ikke anvendt live. Det samlede schema er ikke produktionsklart: frisk lokal replay, pgTAP `plan(105)`, DB-lint, rigtig Auth/Storage/TUS/cleanup-test og review af produktionsdata mangler fortsat.
...
- Projektbundet BEFORE-batch, serverudstedt upload-intent med eksakt pending-row-binding, 6 MiB TUS-chunks, retry-samme-intent og exact-path orphan-reconciliation er implementeret i kilden. Batch-UI'et har ingen egentlig pause- eller reload-resume-kontrol endnu; kun den underliggende AbortSignal-/TUS-kontrakt er testet. Canonical routing til `drone_assets`, evidence size/MIME-kontrakt, cleanup-scheduling samt live RLS-/Storage-/TUS-/cleanup-tests er **AFVENTER**.
...
Kør desuden målrettede tests før hele suiten. Verificeret worktree 2026-09-02, cyklus 014: ren `npm ci --no-audit --no-fund` på Node 22.14.0/npm 10.9.2; typecheck; målrettet ESLint på seks berørte TypeScript-filer; fem målrettede filer/36 tests; en fuld serial kørsel med 54 filer/412 Vitest-tests; normalt produktionsbuild og `git diff --check` består. Migrationskilden har pgTAP `plan(105)`, men replay/pgTAP/DB-lint stopper uden lokal Postgres på `127.0.0.1:54322` og er ikke bestået. De tidligere transaktionelle DB-dry-run-, live tenant A/B SQL- og anonyme PostgREST-gates består fortsat fra cyklus 009. Buildscriptet giver Node 4 GB heap, fordi Nitro-bundlingen overstiger standardheapen. Seneste globale lint er fortsat rød med 5.155 errors/23 warnings og skal ned på 0 før P0-release.
...
- **Orphan-reconciliation fejler:** Kald kun `POST /api/public/monitoring/reconcile-uploads` fra betroet server/cron med den dedikerede `MONITORING_CRON_API_SECRET`. Standardbatch er 25, maksimum 100, og standardlease er 300 sekunder. Workerens databaseclaim er kort; den eksterne Storage-sletning sker bagefter. En fejl frigiver jobbet til retry og gemmer kun saniteret diagnostik. Slet aldrig direkte i `storage.objects`, og markér ikke et claim komplet, hvis Storage API eller completion-RPC fejler. Scheduling/deploy kræver særskilt mandat.
