...
- `20260831064838_harden_4dm_tenant_isolation.sql` er anvendt på den isolerede staging som del af registreret baseline efter atomisk rollback-dry-run og live katalogassertions. Forward migrationen `20260901163924_upload_intents_resumable_storage.sql` er kun i kilden og er ikke anvendt live. Det samlede schema er ikke produktionsklart: frisk lokal replay, pgTAP `plan(83)`, DB-lint, rigtig Auth/Storage/TUS-test og review af produktionsdata mangler fortsat.
...
Kør desuden målrettede tests før hele suiten. Verificeret worktree 2026-09-02, cyklus 013: typecheck; målrettet ESLint på 14 berørte TypeScript-filer; en fuld serial kørsel med 51 filer/397 Vitest-tests; normalt produktionsbuild og `git diff --check` består. Den nye migration har 83 tekstuelt matchede pgTAP-assertions, men hverken replay eller pgTAP er kørt. De tidligere transaktionelle DB-dry-run-, live tenant A/B SQL- og anonyme PostgREST-gates består fortsat fra cyklus 009. Buildscriptet giver Node 4 GB heap, fordi Nitro-bundlingen overstiger standardheapen. Seneste globale lint er fortsat rød med 5.155 errors/23 warnings og skal ned på 0 før P0-release.
