...
`project-media-service.ts` ignorerer persisterede public URLs, validerer at DB-rækkens `file_path` tilhører rækkens projekt, og materialiserer en signed URL med **300 sekunders TTL**. Monitoring-download bruger samme faste 300-sekunders TTL. Upload bruger `upsert: false`; der findes ingen Storage UPDATE-policies i de tre private buckets. Delete slår den autoriserede DB-række op og bruger dens sti i stedet for caller-input. `evidence-service.ts` sanitiserer filnavne, bruger `upsert: false`, gemmer kun den private object-path i `file_url` og forsøger kompensationssletning ved DB-fejl. Evidence-download slår først den eksakte evidence-/projektrække op gennem brugerens RLS-session, accepterer kun en valideret legacy eller canonical privat sti fra rækken og signerer den i 300 sekunder med download-disposition; caller kan ikke levere en Storage-sti. Monitoring-uploadflowet gør en fejlet rollback synlig i stedet for at skjule et efterladt objekt.
...
- Evidence har et eksplicit, fail-closed signed-downloadflow i servicen; rigtig credential-/Storage-/expiry-/revoke-runtimeaccept er fortsat **AFVENTER**.
