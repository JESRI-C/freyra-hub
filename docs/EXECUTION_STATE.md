...
- Verificerede kodecheckpoints: `61bf18b` (runtime/npm/ledger) og `106c825` (endpoint-sikkerhed/env/cutover). Dokumentationscheckpoint og push er **AFVENTER**. Ingen CI, merge, deploy eller produktionsændring er udført.
...
1. Commit P0-styringsdokumenterne og push featurebranchen; provisionér derefter to stærke, uafhængige server-secrets i det relevante miljø og rotér eventuelle historiske credentials efter ejers bekræftelse.
