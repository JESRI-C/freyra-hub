...
Opdateret: 2026-09-12, staging database-/UI-reconciliation og cross-org projektvalg, P0-cyklus 017.

## Seneste verificerede checkpoint

- Repositoryet er `JESRI-C/freyra-hub` på arbejdsbranch `codex/gofreyra-p0`. Commit `fd48949` er pushet til `origin/codex/gofreyra-p0` og udgivet til den navngivne staging-Worker som version `842dbf65-3e99-4fd1-83f5-62360268181d`; produktion og custom domain er ikke ændret.
...
- Den versionsstyrede pgTAP-kilde er nu `plan(107)`, men de 107 cases er **ikke runtime-kørt i denne cyklus**. Aktuel typecheck PASS, målrettet ESLint på ændrede filer PASS, fuld serial Vitest 55 filer/421 tests PASS og staging-build med env-preflight/bundle-check for `xdvqdzdpyceojbdknofi` PASS. Wrangler `--strict --dry-run` PASS, og det efterfølgende staging-deploy PASS med version `842dbf65-3e99-4fd1-83f5-62360268181d`.
- Den hostede UI viser login korrekt, og anonym navigation til `/app` ender på `/login`. Projektvalg skifter nu atomisk til den organisation, som ejer det autoriserede projekt; et projekt-ID uden for brugerens indlæste medlemskaber afvises fortsat. Kilden viser en synlig `Upload`-fane, fører fra en gyldig projektgrænse til en projekt-scopet `FØR`-upload-CTA og stopper flowet ved manglende eller malformed geometri. Kun projektdetalje, projektgeometri og projektkort har eksplicit `ssr:false`; projektindekset har ingen beskyttet loader og ligger under auth-layoutet. Rigtig credential-båret navigation/reload er endnu ikke browserverificeret.
...
2. Typecheck, målrettet ESLint, fuld serial suite 55 filer/421 tests, staging-build/preflight, Wrangler `--strict --dry-run`, staging-deploy og anonym hosted smoke er grønne. Fortsæt den credential-bårne kunderejse på Worker-version `842dbf65-3e99-4fd1-83f5-62360268181d`.
