-- ═══════════════════════════════════════════════════════════════════════════
-- GoFreyra — KOMPLET SETUP (migrations + seed)
-- Paste dette i Supabase SQL Editor og klik Run.
-- Sikkert at køre igen (bruger IF NOT EXISTS og ON CONFLICT DO NOTHING)
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ─── Tjek resultater ─────────────────────────────────────────────────────────
select tabel, raekker from (
  select 'projects'           as tabel, count(*) as raekker from projects           union all
  select 'indicators',                  count(*)             from indicators         union all
  select 'actions',                     count(*)             from actions            union all
  select 'map_layers',                  count(*)             from map_layers         union all
  select 'project_areas',               count(*)             from project_areas      union all
  select 'geo_observations',            count(*)             from geo_observations   union all
  select 'calculated_metrics',          count(*)             from calculated_metrics union all
