...
export function RiskMatrix({
  ...
  points: { name: string; probability: number; impact: number; level: RiskLevel }[];
}) {
  ...
  return (
    <div className="relative aspect-[4/3] w-full">
      ...
      {points.map((p) => (
        ...
        >
          <div
            className="h-3 w-3 rounded-full ring-2 ring-card shadow"
            style={{ background: colors[p.level] }}
          />
      ...
      <div className="absolute -left-2 top-1/2 -translate-x-full -translate-y-1/2 -rotate-90 text-[10px] text-muted-foreground">
        Konsekvens →
      </div>
    </div>
  );
