...
import {
  ArrowRight, CheckCircle2, Circle, AlertTriangle, Sparkles,
  Cable, Brain, Repeat2, BookCheck, FileText, ChevronRight,
} from "lucide-react";
...
export function StatusBadge({ status, className = "" }: { status: PlatformStatus; className?: string }) {
  return (
    <span className={cn("inline-flex items-center gap-1.5 text-xs px-2 py-0.5 rounded-full font-medium", STATUS_TONES[status], className)}>
...
export function ReadinessScore({ value, label = "Rapportklarhed" }: { value: number; label?: string }) {
  const tone = value >= 80 ? "bg-success" : value >= 60 ? "bg-leaf" : value >= 40 ? "bg-warning" : "bg-destructive";
  return (
...
export function ModuleStatusCard({
  id, name, href, metric, sub, status, updated, cta,
}: {
  id: string; name: string; href: string; metric: string; sub: string;
  status: PlatformStatus; updated: string; cta: string;
}) {
  return (
    <Link to={href} className="group block rounded-2xl bg-card border shadow-soft p-4 hover:shadow-card transition">
...
export function DataFlowDiagram({
  ...
  steps: { id: string; name: string; href: string; metric: string; updated: string; status: PlatformStatus }[];
}) {
  return (
    <div className="rounded-2xl bg-card border shadow-soft p-5">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <div className="text-xs text-muted-foreground mt-0.5">Fra rådata til verificeret rapport</div>
        </div>
      ...
      <div className="grid gap-3 md:grid-cols-5">
        {steps.map((s, i) => (
          <div key={s.id} className="relative">
            <Link to={s.href} className="block rounded-xl border bg-background hover:bg-muted/40 transition p-3 h-full">
...
export function ActivityFeed({
  ...
  return (
    <ul className="divide-y">
      {items.map((it, i) => {
        const Icon = it.tone === "warning" ? AlertTriangle : it.tone === "success" ? CheckCircle2 : Sparkles;
        const tone = it.tone === "warning" ? "text-warning-foreground bg-warning/20"
                   : it.tone === "success" ? "text-success bg-success/15"
        ...
        return (
          <li key={i} className="flex items-start gap-3 px-5 py-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs text-muted-foreground mt-0.5">{it.module} · {it.at}</div>
            </div>
          </li>
        );
      })}
...
export function CriticalActionsPanel({
  ...
  items: { module: string; title: string; priority: string; owner: string; deadline: string; href: string }[];
}) {
  return (
    <ul className="divide-y">
      {items.map((a, i) => {
        const tone = a.priority === "Høj" ? "bg-destructive/15 text-destructive"
                   : a.priority === "Medium" ? "bg-warning/20 text-warning-foreground"
                   : "bg-muted text-muted-foreground";
        return (
          <li key={i} className="px-5 py-3 flex items-center gap-3">
            <div className={cn("text-[11px] font-medium px-2 py-0.5 rounded-full", tone)}>{a.priority}</div>
            <div className="flex-1 min-w-0">
            ...
            <Link to={a.href} className="text-sm text-primary inline-flex items-center gap-1 hover:underline">
    ...
    <div className="rounded-2xl bg-card border shadow-soft p-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="text-xs text-muted-foreground mt-0.5">{done} af {steps.length} trin gennemført</div>
        </div>
...
export function EmptyStateCard({
  title, description, ctaLabel, ctaHref, icon,
}: {
  title: string; description: string; ctaLabel?: string; ctaHref?: string; icon?: ReactNode;
}) {
  return (
    <div className="rounded-2xl border border-dashed bg-card p-10 text-center">
      {icon && <div className="h-10 w-10 mx-auto rounded-xl bg-leaf/20 text-primary grid place-items-center mb-3">{icon}</div>}
      <div className="font-semibold">{title}</div>
      <p className="text-sm text-muted-foreground mt-1.5 max-w-md mx-auto">{description}</p>
      {ctaLabel && ctaHref && (
        <Link to={ctaHref} className="inline-flex items-center gap-2 mt-4 rounded-xl bg-primary text-primary-foreground px-4 py-2 text-sm font-medium shadow-soft hover:opacity-95">
...
export function CrossModuleLink({ to, label }: { to: string; label: string }) {
  return (
    <Link to={to} className="inline-flex items-center gap-1 text-xs px-2 py-1 rounded-lg border bg-background hover:bg-muted text-foreground transition">
