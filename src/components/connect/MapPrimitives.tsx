...
import {
  Search, PencilLine, Square, Edit3, Ruler, UploadCloud, Save, Download,
  X, MapPin, Layers as LayersIcon, CheckCircle2, AlertTriangle, Sparkles,
  FileText, ArrowRight, Send, Plus, Eye, Crosshair,
} from "lucide-react";
import { Card, Pill } from "@/components/ui-bits";
import { Chip, DataQualityScore } from "./Primitives";
import {
  LAYER_DEFS, MOCK_SENSORS, MOCK_FIELD_OBS, MOCK_WATER_SAMPLES, MOCK_ZONES_GEO,
  SEARCH_SUGGESTIONS, MAP_PROJECTS, COVERAGE_METRICS, ROUTING_DESTINATIONS,
  type LayerKey,
...
export function MapToolbar({
  active, onTool, areaText,
}: { active: ToolKey | null; onTool: (t: ToolKey) => void; areaText?: string }) {
...
export function MapSearchBox({ onPick }: { onPick: (s: string) => void }) {
  ...
  const filtered = SEARCH_SUGGESTIONS.filter((s) => s.label.toLowerCase().includes(q.toLowerCase()));
  return (
      ...
      {open && filtered.length > 0 && (
        <ul className="absolute z-20 left-0 right-0 mt-1 bg-card border rounded-lg shadow-card overflow-hidden">
          {filtered.map((s) => (
            <li key={s.label}>
              <button
                onClick={() => { onPick(s.label); setQ(s.label); setOpen(false); }}
                className="w-full text-left px-3 py-2 text-sm hover:bg-muted flex items-center justify-between"
              >
                <span className="inline-flex items-center gap-2"><MapPin className="h-3.5 w-3.5 text-primary" /> {s.label}</span>
                <span className="text-[11px] text-muted-foreground">{s.type} · {s.coord}</span>
              </button>
            </li>
          ))}
        </ul>
...
export function LayerControlPanel({
  active, onToggle,
}: { active: Record<LayerKey, boolean>; onToggle: (k: LayerKey, v: boolean) => void }) {
  return (
    <div>
            ...
            <label className="flex items-start gap-2 px-2.5 py-2 cursor-pointer">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[11px] text-muted-foreground flex items-center justify-between gap-2">
                  <span>{l.source} · {l.lastUpdated}</span>
  ...
  return (
    <div className="relative h-[560px] bg-gradient-to-br from-leaf/30 via-card to-leaf/10 overflow-hidden">
      <svg viewBox="0 0 600 400" className={`w-full h-full ${drawing ? "cursor-crosshair" : ""}`} onClick={handleClick} onDoubleClick={onDoubleClick}>
        ...
        {layers.drone && (
          <path d="M70,70 L420,75 L440,260 L80,250 Z" fill="oklch(0.7 0.1 200 / 0.15)" stroke="oklch(0.55 0.12 210)" strokeOpacity="0.4" strokeDasharray="3 3" />
        )}

        {/* Project boundary */}
        {layers.boundary && (
          <path d="M60,60 L520,80 L540,300 L80,340 Z"
        ...
        {/* Zones */}
        {layers.zones && MOCK_ZONES_GEO.map((z, i) => (
          <g key={z.name} className="cursor-pointer" onClick={() => onPick({ kind: "zone", data: z })}>
            <path d={z.path} fill={`oklch(0.55 0.13 155 / ${0.06 + i * 0.02})`} stroke="var(--primary)" strokeOpacity="0.4" />
          </g>
        ))}

        {/* Zone labels */}
        {layers.zones && MOCK_ZONES_GEO.map((z) => {
          const m = z.path.match(/M(\d+),(\d+)/);
          if (!m) return null;
          const x = +m[1] + 60, y = +m[2] + 60;
          return <text key={z.name + "l"} x={x} y={y} fontSize="10" className="fill-foreground pointer-events-none" opacity="0.75">{z.name.split("—")[0].trim()}</text>;
        })}

        {/* Data gaps */}
        {layers.gaps && (
          <>
            <g className="cursor-pointer" onClick={() => onPick({ kind: "gap", data: { label: "Datagap nordøst" } })}>
              <rect x={420} y={235} width="80" height="60" fill="oklch(0.78 0.14 60 / 0.25)" stroke="oklch(0.6 0.18 50)" strokeDasharray="4 3" />
              <text x={428} y={270} fontSize="10" fill="oklch(0.45 0.18 50)">Datagap</text>
            </g>
            <g className="cursor-pointer" onClick={() => onPick({ kind: "gap", data: { label: "Drone gap nordøst" } })}>
              <rect x={460} y={90} width="60" height="40" fill="oklch(0.78 0.14 60 / 0.2)" stroke="oklch(0.6 0.18 50)" strokeDasharray="4 3" />
            </g>
          </>
        )}
        ...
        {layers.sensors && MOCK_SENSORS.map((s) => (
          <g key={s.id} className="cursor-pointer" onClick={() => onPick({ kind: "sensor", data: s })}>
        ...
        {/* Gateway */}
        {layers.gateways && (
          <g className="cursor-pointer" onClick={() => onPick({ kind: "gateway", data: { id: "SKB-GW-03", label: "LoRaWAN Gateway 03" } })}>
            <rect x={290} y={170} width="14" height="14" fill="oklch(0.55 0.2 25)" />
            <text x={310} y={182} fontSize="10" fill="oklch(0.5 0.2 25)">GW-03 offline</text>
          </g>
        )}

        {/* Field obs */}
        {layers.field && MOCK_FIELD_OBS.map((f, i) => (
          <g key={i} className="cursor-pointer" onClick={() => onPick({ kind: "field", data: f })}>
            <polygon points={`${f.x},${f.y - 7} ${f.x + 6},${f.y + 5} ${f.x - 6},${f.y + 5}`} fill="var(--leaf)" />
          </g>
        ))}

        {/* Water samples */}
        {layers.water && MOCK_WATER_SAMPLES.map((w, i) => (
          <g key={i} className="cursor-pointer" onClick={() => onPick({ kind: "water", data: w })}>
            <circle cx={w.x} cy={w.y} r="6" fill="oklch(0.6 0.15 230)" stroke="white" strokeWidth="1.5" />
          </g>
        ))}

        {/* Alerts */}
        {layers.alerts && (
          <g className="cursor-pointer" onClick={() => onPick({ kind: "alert", data: { id: "ALT-2041", label: "Sensor Gateway 03 offline" } })}>
      ...
      {drawing && (
        <div className="absolute top-3 left-1/2 -translate-x-1/2 inline-flex items-center gap-2 text-xs bg-foreground text-background rounded-full px-3 py-1.5 shadow-card">
          <Crosshair className="h-3.5 w-3.5" /> Klik på kortet for at tegne zonen. Dobbeltklik for at afslutte.
        </div>
      )}

      {/* Mini legend */}
      <div className="absolute bottom-3 right-3 bg-card/95 border rounded-lg p-2.5 text-[11px] space-y-1 shadow-soft">
        <div className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-primary" /> Sensor</div>
        <div className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-sm" style={{ background: "oklch(0.55 0.2 25)" }} /> Gateway / Alert</div>
        <div className="flex items-center gap-1.5"><span className="h-2 w-2 rotate-45 bg-leaf" /> Feltobservation</div>
        <div className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full" style={{ background: "oklch(0.6 0.15 230)" }} /> Vandprøve</div>
        <div className="flex items-center gap-1.5"><span className="h-2 w-2" style={{ background: "oklch(0.78 0.14 60)" }} /> Datagap</div>
      </div>
    </div>
  );
}
...
// ---------- Zone card ----------
export function ZoneCard({ z, onOpen }: { z: typeof MOCK_ZONES_GEO[number]; onOpen: () => void }) {
  return (
    <div className="rounded-xl border bg-card p-4 hover:shadow-soft transition">
      <div className="flex items-start justify-between gap-2">
        <div>
          ...
          <div className="text-xs text-muted-foreground mt-0.5">{z.area} · {z.habitat}</div>
        </div>
      ...
      <button onClick={onOpen} className="mt-3 w-full text-xs rounded-lg border bg-card hover:bg-muted px-3 py-1.5 inline-flex items-center justify-center gap-1.5">
...
}: { selection: MapSelection | null; onClose: () => void; onAction: (label: string) => void }) {
  if (!selection) {
    return (
      <div className="rounded-xl border bg-card p-5 text-sm text-muted-foreground">
        <div className="font-semibold text-foreground mb-1">Intet valgt</div>
        Klik på en zone, sensor, gateway, observation, vandprøve, datagap eller alert på kortet for at se detaljer.
      </div>
    );
  }
      ...
      <div className="p-4 space-y-3 text-sm">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {blocks.modules.map((m) => <Chip key={m} tone="primary">{m}</Chip>)}
          </div>
        </div>
        {blocks.related.length > 0 && (
          <div>
            ...
            <ul className="text-xs text-muted-foreground space-y-1">
              {blocks.related.map((r) => <li key={r}>· {r}</li>)}
            </ul>
          </div>
        )}
      </div>
      <div className="p-3 border-t bg-muted/30 flex flex-wrap gap-1.5">
        {["Kobl datakilde", "Send til ESG Ledger", "Brug i rapport", "Se observationer", "Redigér"].map((a) => (
          <button key={a} onClick={() => onAction(a)} className="text-xs rounded-lg border bg-card hover:bg-muted px-2.5 py-1">{a}</button>
        ))}
      </div>
    </div>
  );
...
function describeSelection(s: MapSelection) {
  switch (s.kind) {
    case "zone":
      return {
        kind: "Zone", title: s.data.name, subtitle: `${s.data.area} · ${s.data.habitat}`,
        kv: [
          { k: "Areal", v: s.data.area }, { k: "Habitat", v: s.data.habitat },
          { k: "Datakvalitet", v: <DataQualityScore score={s.data.quality} /> },
          { k: "Sidste observation", v: s.data.lastObs },
          { k: "Datakilder", v: `${s.data.sources}` }, { k: "Risiko", v: s.data.risk },
        ],
    ...
    case "sensor":
      return {
        kind: "Sensor", title: s.data.id, subtitle: s.data.label,
        kv: [
          { k: "Zone", v: s.data.zone }, { k: "Datakvalitet", v: <DataQualityScore score={s.data.quality} /> },
          { k: "Status", v: <Pill tone="success">Online</Pill> }, { k: "Koordinater", v: "55.252, 9.488" },
        ],
    ...
    case "field":
      return {
        kind: "Feltobservation", title: s.data.label, subtitle: `Observeret ${s.data.date}`,
        kv: [{ k: "Type", v: "Artsregistrering" }, { k: "Kilde", v: "Field App" }, { k: "Verifikation", v: "Afventer" }, { k: "Koordinater", v: "55.250, 9.491" }],
        modules: ["Impact Exchange"], related: ["Verifikationsflow åbent"],
      };
    case "water":
      return {
        kind: "Vandprøve", title: s.data.label, subtitle: `pH ${s.data.ph}`,
        kv: [{ k: "pH", v: s.data.ph }, { k: "Kilde", v: "Lab" }, { k: "Koordinater", v: "55.251, 9.489" }, { k: "Verifikation", v: "Verificeret" }],
        modules: ["ESG Ledger", "Reports"], related: ["Vandkvalitetsrapport Q2"],
      };
    case "gateway":
      return {
        kind: "Gateway", title: s.data.id, subtitle: s.data.label,
        kv: [{ k: "Status", v: <Pill tone="danger">Offline</Pill> }, { k: "Sidste kontakt", v: "3 t siden" }, { k: "Connection", v: "LoRaWAN" }, { k: "Koordinater", v: "55.252, 9.490" }],
        modules: [], related: ["Alert ALT-2041 åben"],
      };
    case "alert":
      return {
        kind: "Alert", title: s.data.id, subtitle: s.data.label,
        kv: [{ k: "Severity", v: <Pill tone="danger">Kritisk</Pill> }, { k: "Status", v: "Åben" }, { k: "Owner", v: "Mikkel Holm" }, { k: "Oprettet", v: "I dag 09:14" }],
        modules: ["Smart Connect"], related: ["Sensor Gateway 03"],
      };
    case "gap":
      return {
        kind: "Datagap", title: s.data.label, subtitle: "Manglende dækning",
        kv: [{ k: "Areal", v: "0,9 ha" }, { k: "Mangler", v: "Drone + felt" }, { k: "Sidste data", v: "12 dage siden" }, { k: "Påvirker", v: "Rapportklarhed" }],
        modules: ["Reports"], related: ["DecisionsIQ: Prioritér feltbesøg"],
      };
  }
}
...
export function PolygonDrawModal({
  open, areaText, onClose, onSave,
}: { open: boolean; areaText: string; onClose: () => void; onSave: (z: { name: string; type: string; habitat: string }) => void }) {
      ...
      <div className="w-full max-w-md bg-card rounded-2xl border shadow-card overflow-hidden">
        ...
        <div className="p-5 space-y-3">
          <Field label="Zonenavn">
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="F.eks. Zone E — Strandeng" className="w-full rounded-lg border bg-background px-3 py-2 text-sm" />
          </Field>
          <Field label="Zonetype">
            <select value={type} onChange={(e) => setType(e.target.value)} className="w-full rounded-lg border bg-background px-3 py-2 text-sm">
              {["Habitat zone", "Buffer", "Måleområde", "Restoration"].map((t) => <option key={t}>{t}</option>)}
            </select>
          </Field>
          <Field label="Habitattype">
            <select value={habitat} onChange={(e) => setHabitat(e.target.value)} className="w-full rounded-lg border bg-background px-3 py-2 text-sm">
              {["Vandløb", "Eng og vådområde", "Skovkant", "Buffer", "Hav/kyst", "Strandeng"].map((t) => <option key={t}>{t}</option>)}
            </select>
          </Field>
          <Field label="Tilknyt datakilder">
            <div className="flex flex-wrap gap-1.5">
              {["Sensorer", "Drone Q2", "Sentinel-2 NDVI", "Field App", "Vandprøver"].map((s) => (
                <label key={s} className="text-xs rounded-full border px-2.5 py-1 bg-muted/40 inline-flex items-center gap-1.5">
        ...
        <div className="p-4 border-t bg-muted/30 flex justify-end gap-2">
          <button onClick={onClose} className="text-xs rounded-lg border bg-card px-3 py-1.5">Annullér</button>
          <button
...
export function AreaMeasurementBadge({ ha }: { ha: number }) {
  ...
  return (
    <div className="rounded-xl border bg-card p-4">
      ...
      <div className="text-xs text-muted-foreground mt-1">{m2} m² · {km2} km²</div>
    </div>
  );
}
...
export function UploadDropzone({ onAdd }: { onAdd: (files: { name: string; size: string; type: string }[]) => void }) {
  ...
  return (
    ...
    >
      ...
      <div className="text-xs text-muted-foreground mt-1">Accepterede: .tif, .geotiff, .geojson, .kml, .zip, .csv, .gpx, .jpg, .png, .pdf</div>
      <div className="mt-4 flex justify-center gap-2 flex-wrap">
        <button onClick={() => ref.current?.click()} className="text-xs rounded-lg bg-primary text-primary-foreground px-3 py-1.5 inline-flex items-center gap-1.5">
          <UploadCloud className="h-3.5 w-3.5" /> Upload filer
        </button>
        <button onClick={() => onAdd([{ name: "imported_zones.geojson", size: "8 KB", type: "GeoJSON" }])} className="text-xs rounded-lg border bg-card px-3 py-1.5 inline-flex items-center gap-1.5">
          <Plus className="h-3.5 w-3.5" /> Importér GeoJSON
        </button>
        <button onClick={() => onAdd([{ name: "manuel_feltdata_2026.csv", size: "12 KB", type: "Feltdata" }])} className="text-xs rounded-lg border bg-card px-3 py-1.5 inline-flex items-center gap-1.5">
...
export function FileMetadataPanel({ name, type }: { name: string; type: string }) {
  const fields = [
    "Projekt", "Zone", "Datatype", "Indsamlingsdato", "Datakilde / ejer",
    "Metode", "Koordinatsystem (EPSG)", "Opløsning", "Sensor / kameratype",
  ];
  return (
    <div className="rounded-xl border bg-card p-4">
      ...
      <div className="grid sm:grid-cols-2 gap-2.5">
        {fields.map((f) => (
          <label key={f} className="block">
            ...
            <input className="w-full rounded-lg border bg-background px-2.5 py-1.5 text-xs" placeholder={f} />
          </label>
        ))}
        <label className="block sm:col-span-2">
          <div className="text-[11px] text-muted-foreground mb-1">Beskrivelse</div>
          <textarea rows={2} className="w-full rounded-lg border bg-background px-2.5 py-1.5 text-xs" placeholder="Kort beskrivelse af datasæt og kontekst" />
        </label>
        <label className="block sm:col-span-2">
          <div className="text-[11px] text-muted-foreground mb-1">Datakvalitetsnote</div>
          <textarea rows={2} className="w-full rounded-lg border bg-background px-2.5 py-1.5 text-xs" placeholder="Eventuelle forbehold, kalibrering eller usikkerhed" />
        </label>
      </div>
...
export function GeospatialValidationPanel({ checks }: { checks?: { label: string; pass: boolean }[] }) {
  const list = checks ?? [
      ...
      <ul className="grid sm:grid-cols-2 gap-1.5 text-xs">
        {list.map((c) => (
          <li key={c.label} className="flex items-center gap-2 p-2 rounded-lg border bg-muted/30">
            {c.pass ? <CheckCircle2 className="h-3.5 w-3.5 text-success" /> : <AlertTriangle className="h-3.5 w-3.5 text-warning-foreground" />}
    ...
    <div className="rounded-xl border bg-card p-4">
      ...
      <div className="rounded-lg border bg-gradient-to-br from-leaf/30 via-card to-leaf/10 overflow-hidden">
        <svg viewBox="0 0 200 120" className="w-full h-32">
          ...
          <path d="M20,20 L180,25 L185,100 L25,105 Z" fill="none" stroke="var(--primary)" strokeDasharray="4 3" />
        ...
        return (
          ...
          >
            <div className="flex items-start justify-between">
              ...
              <span className={`h-4 w-4 rounded border grid place-items-center ${active ? "bg-primary border-primary text-primary-foreground" : "bg-card"}`}>
...
export function UploadQueueTable({
  rows, onSelect, selectedId, onAction,
}: {
  rows: { id: string; name: string; type: string; size: string; project: string; zone: string; status: string; geo: boolean; metaOk: boolean; validationNote: string }[];
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y">
            {rows.map((r) => (
              <tr key={r.id} className={`hover:bg-muted/40 cursor-pointer ${selectedId === r.id ? "bg-leaf/10" : ""}`} onClick={() => onSelect(r.id)}>
                ...
                <td className="px-4 py-3">
                  <Pill tone={r.status === "Klar" ? "success" : r.status === "Fejl" ? "danger" : "warning"}>{r.status}</Pill>
                </td>
                <td className="px-4 py-3">{r.geo ? <Chip tone="primary">Ja</Chip> : <Chip>Nej</Chip>}</td>
                <td className="px-4 py-3">{r.metaOk ? <Chip tone="primary">OK</Chip> : <Chip tone="muted">Mangler</Chip>}</td>
                <td className="px-4 py-3 text-xs text-muted-foreground">{r.validationNote}</td>
                <td className="px-4 py-3 text-right">
                  <button onClick={(e) => { e.stopPropagation(); onAction("Preview on map", r.id); }} className="text-xs rounded-lg border bg-card px-2 py-1 inline-flex items-center gap-1">
