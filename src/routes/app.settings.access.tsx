...
const GROUPS = [
  ...
  {
    name: "AI & rapporter",
    perms: ["Brug DecisionsIQ", "Eksportér rapporter", "Godkend rapporter"],
  },
  {
    name: "Administration",
    perms: ["Administrér brugere", "Administrér abonnement", "API-adgang"],
  },
];
    ...
    <main className="max-w-[1400px] mx-auto px-6 py-6 space-y-6">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold">Adgang & rettigheder</h1>
            <p className="text-xs text-muted-foreground">Definér hvad hver rolle kan se og gøre</p>
          </div>
        </div>
        <button
        ...
        >
          <Save className="h-4 w-4" /> Gem rolledefinition
        </button>
      </div>
      ...
      <div className="rounded-xl border bg-leaf/5 p-3 text-xs flex gap-2 items-start">
        ...
        <div>
          Klik på en celle for at skifte mellem{" "}
          <span className="inline-flex items-center gap-1">
            <span className="h-3 w-3 rounded bg-success" /> fuld
          </span>
          ,{" "}
          <span className="inline-flex items-center gap-1">
            <span className="h-3 w-3 rounded bg-warning" /> begrænset
          </span>{" "}
          og{" "}
          <span className="inline-flex items-center gap-1">
            <span className="h-3 w-3 rounded bg-muted-foreground/40" /> ingen
          </span>{" "}
          adgang.
        </div>
      </div>
            ...
            <thead className="bg-muted/40 text-xs text-muted-foreground sticky top-0">
              <tr>
                ...
                {PERMISSIONS.map((p) => (
                  <th
                    key={p}
                    className="font-medium px-2 py-2.5 text-[11px] text-center whitespace-nowrap"
                  >
                    {p}
                  </th>
                ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Section
          title={`Tilladelser for ${activeRole}`}
          subtitle="Grupperet visning"
