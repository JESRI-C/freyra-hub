    ...
    <main className="p-6 max-w-[1400px] w-full mx-auto space-y-5">
      <PageHeader
        ...
        actions={
          <button
            onClick={() =>
              actionToast("Anbefalingsliste eksporteret", "Filen er klar i Eksporter-historik")
            }
            className="text-sm rounded-lg border bg-card px-3 py-2"
          >
            Eksportér liste
          </button>
        }
      ...
      <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        />
        <StatCard
          label="Kan dokumenteres direkte"
          value="7"
          icon={<FileCheck2 className="h-5 w-5" />}
          accent="bg-success/15 text-success"
        />
        <StatCard
          label="Kræver bedre datagrundlag"
          value="3"
      ...
      <Card className="p-4">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="ml-auto text-xs text-muted-foreground">
            {filtered.length} af {RECOMMENDATIONS.length}
          </div>
        </div>
      </Card>
