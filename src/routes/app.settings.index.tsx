    ...
    <main className="max-w-[1400px] mx-auto px-6 py-6 space-y-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-3">
          <div className="h-12 w-12 rounded-2xl grid place-items-center bg-leaf/30 text-primary">
            <Building2 className="h-6 w-6" />
          </div>
          <div>
            <h1 className="text-xl font-semibold">{p.name}</h1>
            <p className="text-xs text-muted-foreground">
              {p.cvr} · {p.country} · Plan: Professional
            </p>
          </div>
        </div>
        <button
        ...
        >
          <Save className="h-4 w-4" /> Gem ændringer
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Section
          title="Organisationslogo"
          subtitle="Bruges på rapporter og deling"
          className="lg:col-span-1"
        >
          <div className="rounded-2xl border-2 border-dashed bg-muted/30 p-8 text-center">
            <div className="h-16 w-16 mx-auto rounded-2xl bg-leaf grid place-items-center text-leaf-foreground">
          ...
          <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
            <div className="rounded-lg border p-2">
              <div className="text-muted-foreground">Primær farve</div>
              <div className="mt-1 flex items-center gap-2">
                <span className="h-4 w-4 rounded bg-primary" /> Skov-grøn
              </div>
            </div>
            <div className="rounded-lg border p-2">
              <div className="text-muted-foreground">Accent</div>
              <div className="mt-1 flex items-center gap-2">
                <span className="h-4 w-4 rounded bg-leaf" /> Blad
              </div>
            </div>
          </div>
        </Section>
        ...
        <Section title="Virksomhedsoplysninger" className="lg:col-span-2">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <Select
              label="Land"
              value={p.country}
              options={["Danmark", "Sverige", "Norge", "Tyskland", "Indonesien", "Kenya"]}
        ...
        <Section title="Rapportering & regulativ kontekst">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <Field
              ...
              value={p.reportingYear}
              onChange={update("reportingYear")}
            />
            <Field
              label="Rapporteringsperiode"
              value={p.reportingPeriod}
              onChange={update("reportingPeriod")}
            />
            <Select
              label="Standard rapportsprog"
              value={p.language}
            ...
            <Select
              ...
              value={p.currency}
              options={["DKK", "EUR", "USD", "SEK", "NOK"]}
              onChange={update("currency")}
            />
            <Select
              label="Tidszone"
              value={p.timezone}
              options={["Europe/Copenhagen", "Europe/Stockholm", "UTC"]}
              onChange={update("timezone")}
            />
            <Select
              label="Regulativ kontekst"
              value="EU — CSRD/ESRS"
              options={["EU — CSRD/ESRS", "Global — voluntary", "Local — kommune"]}
            />
          </div>
        </Section>
      </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Freyra er databehandler. Din organisation er dataansvarlig. Data lagres i EU
              (Frankfurt) og er beskyttet af DPA-aftale, kryptering i hvile og under transport, samt
              audit logging i ESG Ledger.
            </p>
            <div className="mt-2 flex gap-2">
              <button className="text-xs rounded-lg border bg-card px-2.5 py-1">Hent DPA</button>
              <button className="text-xs rounded-lg border bg-card px-2.5 py-1">
                Se sikkerhedsdokument
              </button>
            </div>
          </div>
        </div>
