...
import { Cpu, Wifi, BatteryLow, WifiOff, Network, Wrench, Sparkles, RefreshCw, Activity, Power, Send } from "lucide-react";
import { Card, PageHeader } from "@/components/ui-bits";
import {
  ConnectionHealthCard, DeviceStatusBadge, SignalStrengthIndicator, BatteryIndicator,
  DataQualityScore, Drawer, MiniLine, Section, Chip,
...
function Page() {
  ...
  return (
    <main className="p-6 max-w-[1400px] w-full mx-auto space-y-4">
      <PageHeader title="Enheder" description="Sensorer, gateways, kameraer, vejrstationer og akustiske monitorer." />

      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        <ConnectionHealthCard label="Enheder" value="28" icon={<Cpu className="h-5 w-5" />} />
        <ConnectionHealthCard label="Online" value="23" tone="success" icon={<Wifi className="h-5 w-5" />} />
        <ConnectionHealthCard label="Lavt batteri" value="3" tone="warning" icon={<BatteryLow className="h-5 w-5" />} />
        ...
        <ConnectionHealthCard label="Firmware-opd." value="4" tone="warning" icon={<Wrench className="h-5 w-5" />} />
      </div>
      ...
      <Card className="p-4">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {[
            ["all", "Alle"], ["online", "Online"], ["partial", "Delvist"],
            ["attention", "Kræver handling"], ["offline", "Offline"],
          ].map(([k, l]) => (
            <button
              key={k}
              onClick={() => setFilter(k)}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y">
              {filtered.map((d) => (
                <tr key={d.id} className="hover:bg-muted/40 cursor-pointer" onClick={() => setSelected(d)}>
                  ...
                  <td className="px-4 py-3"><BatteryIndicator level={d.battery} /></td>
                  <td className="px-4 py-3"><SignalStrengthIndicator level={d.signal} /></td>
                  <td className="px-4 py-3 text-xs">{d.lastContact}</td>
                  <td className="px-4 py-3 text-xs"><Chip>{d.firmware}</Chip></td>
                  <td className="px-4 py-3 text-xs">{d.lastReading}</td>
                  <td className="px-4 py-3"><DataQualityScore score={d.quality} /></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <Section title="AI vedligeholdelsesanbefalinger" subtitle="Prioritér service og kalibrering">
        <ul className="space-y-2 text-sm">
          <li className="flex gap-3 p-3 rounded-lg border bg-muted/30">
            ...
            <div><strong>SKB-GW-03</strong> — Kontrollér strømforsyning og LoRaWAN backhaul. Offline i 3 timer påvirker 6 underliggende sensorer.</div>
          </li>
          <li className="flex gap-3 p-3 rounded-lg border bg-muted/30">
            <Sparkles className="h-4 w-4 text-primary mt-0.5" />
            <div><strong>WET-CAM-02</strong> — Batteri 18%. Planlæg fysisk besøg inden for 5 dage for at undgå datatab i Zone D.</div>
          </li>
          <li className="flex gap-3 p-3 rounded-lg border bg-muted/30">
            <Sparkles className="h-4 w-4 text-primary mt-0.5" />
            <div><strong>SKB-WQ-01</strong> — 6-måneders kalibreringsvindue nås om 14 dage. Bestil kalibreringskit nu.</div>
          </li>
        </ul>
      </Section>
      ...
      <Drawer
        ...
        footer={
          <>
            ...
            <button className="text-xs rounded-lg border bg-card px-3 py-1.5 inline-flex items-center gap-1.5"><Activity className="h-3.5 w-3.5" /> Se live data</button>
            <button className="text-xs rounded-lg border bg-card px-3 py-1.5 inline-flex items-center gap-1.5"><Wrench className="h-3.5 w-3.5" /> Planlæg service</button>
            <button className="text-xs rounded-lg border bg-card px-3 py-1.5 inline-flex items-center gap-1.5"><Send className="h-3.5 w-3.5" /> Send til ESG Ledger</button>
            <button className="text-xs rounded-lg border bg-card text-destructive px-3 py-1.5 inline-flex items-center gap-1.5"><Power className="h-3.5 w-3.5" /> Deaktiver</button>
          </>
        }
      >
        {selected && (
          <>
            ...
            <Section title="Batteri & signal trend" subtitle="Sidste 14 dage">
              <MiniLine values={[80, 78, 75, 73, 71, 70, 68, 66, 65, 63, 60, 58, 56, selected.battery]} />
            </Section>
            <Section title="Linkede datastreams">
              <ul className="text-sm space-y-1.5">
                <li className="flex justify-between"><span>Datastream — pH</span><Chip tone="primary">aktiv</Chip></li>
                <li className="flex justify-between"><span>Datastream — Temperatur</span><Chip tone="primary">aktiv</Chip></li>
                <li className="flex justify-between"><span>Datastream — Turbiditet</span><Chip>pauset</Chip></li>
              </ul>
            </Section>
