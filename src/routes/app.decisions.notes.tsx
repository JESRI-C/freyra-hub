...
function Page() {
  ...
  return (
    <main className="p-6 max-w-[1400px] w-full mx-auto space-y-5">
      <PageHeader
        title="Beslutningsnotater"
        description="Generér professionelle beslutningsnotater baseret på AI-analyse."
      />
        ...
        <div className="space-y-4">
          <Card className="p-5">
            ...
            <div className="space-y-3">
              ...
              <FormField label="Målgruppe">
                <Select value={audience} onChange={setAudience} options={AUDIENCES} />
              </FormField>
              <FormField label="Fokus">
                <Select value={focus} onChange={setFocus} options={FOCUS} />
              </FormField>
          ...
          <Card className="p-4">
            <div className="text-xs text-muted-foreground">
              Notater bruger projektets verificerede data og kan tilføjes til ESG Ledger som
              permanent post.
            </div>
          </Card>
        </div>
        ...
        <Card>
          <div className="px-5 py-3 border-b flex items-center justify-between flex-wrap gap-2">
            ...
            <div className="flex flex-wrap gap-2">
              <button className="inline-flex items-center gap-1.5 text-xs rounded-lg border bg-card px-2.5 py-1.5">
                <Download className="h-3.5 w-3.5" /> Eksportér PDF
              </button>
              <button className="inline-flex items-center gap-1.5 text-xs rounded-lg border bg-card px-2.5 py-1.5">
                <Send className="h-3.5 w-3.5" /> Send til rapporter
              </button>
              <button className="inline-flex items-center gap-1.5 text-xs rounded-lg border bg-card px-2.5 py-1.5">
      ...
      <Card>
        <CardHeader
          title="Gemte notater"
          subtitle="Bibliotek over tidligere genererede beslutningsnotater"
        />
        <table className="w-full text-sm">
          <thead className="text-left text-xs text-muted-foreground border-y bg-muted/40">
            <tr>
              ...
              <th className="py-2">Målgruppe</th>
            ...
            {SAVED_NOTES.map((n) => (
              <tr key={n.id}>
                <td className="px-5 py-3 font-medium flex items-center gap-2">
                  <FileText className="h-4 w-4 text-muted-foreground" /> {n.title}
                </td>
                ...
                <td className="pr-5 text-right">
                  <button className="text-sm text-primary hover:underline">Åbn</button>
                </td>
              </tr>
            ))}
          </tbody>
