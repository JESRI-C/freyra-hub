...
function Page() {
  ...
  const send = (text: string) => {
    const t = text.trim();
    if (!t) return;
    setMessages((m) => [...m, { role: "user", text: t }, { role: "assistant", reply: pickReply(t) }]);
    setInput("");
  ...
  return (
    <main className="p-6 max-w-[1400px] w-full mx-auto space-y-5">
      <PageHeader title="AI-assistent" description="Stil spørgsmål til dit projekt — svar baseres kun på tilgængelige, verificerede data." />
      ...
      <div className="grid lg:grid-cols-[1fr_320px] gap-5">
        <Card className="flex flex-col h-[620px]">
            ...
            {messages.map((m, i) => (
              m.role === "user" ? (
                <div key={i} className="flex justify-end">
                  <div className="max-w-[80%] rounded-2xl rounded-tr-sm bg-primary text-primary-foreground px-4 py-2.5 text-sm">{m.text}</div>
                </div>
              ) : (
                <div key={i} className="flex gap-3">
                  <div className="h-8 w-8 rounded-full bg-leaf/20 text-primary grid place-items-center shrink-0"><Sparkles className="h-4 w-4" /></div>
                    ...
                    <div className="grid sm:grid-cols-2 gap-2 text-xs">
                      <Block label="Datagrundlag" tone="info">
                        <ul className="space-y-1">
                          {m.reply.basis.map((b) => <li key={b} className="flex gap-1.5"><span className="h-1 w-1 rounded-full bg-leaf mt-1.5" />{b}</li>)}
                        </ul>
                      </Block>
                      <Block label="Anbefaling" tone="success">{m.reply.recommendation}</Block>
                      <Block label="Usikkerhed" tone="warning">{m.reply.uncertainty}</Block>
                      <Block label="Næste handling" tone="primary">{m.reply.nextAction}</Block>
                    </div>
                  </div>
                </div>
              )
          ...
          >
            ...
            />
            <button type="submit" className="inline-flex items-center gap-2 rounded-xl bg-primary text-primary-foreground px-4 text-sm font-medium shadow-soft">
        ...
        <div className="space-y-4">
          <Card>
            ...
            <ul className="px-5 pb-5 space-y-3 text-sm">
              ...
              <Ctx icon={<Database className="h-4 w-4" />} label="Forbundne kilder" value="Sensorer, Sentinel-2, Felt, ESG ledger" />
              <Ctx icon={<Sparkles className="h-4 w-4" />} label="Datakonfidens" value={<ConfidenceScore value={0.82} size="sm" />} />
          ...
          <Card className="p-4">
            <div className="flex gap-3">
              ...
              <p className="text-xs text-muted-foreground leading-relaxed">
                AI-assistenten bruger projektets tilgængelige data og bør anvendes som beslutningsstøtte, ikke som endelig
                verifikation. Verificeret dokumentation sker via ESG Ledger.
              </p>
            </div>
          </Card>
        </div>
...
function Block({ label, children, tone }: { label: string; children: React.ReactNode; tone: "info" | "success" | "warning" | "primary" }) {
