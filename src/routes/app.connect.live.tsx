  ...
  return (
    <main className="p-6 max-w-[1400px] w-full mx-auto space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
        <ConnectionHealthCard
          label="Observationer i dag"
          value="1.284"
          icon={<Activity className="h-5 w-5" />}
        />
        <ConnectionHealthCard
          label="Aktive datastreams"
          value="42"
          tone="success"
      ...
      <Card className="p-4">
        <div className="flex flex-wrap items-center gap-3">
          <Select
            ...
            value={project}
            onChange={setProject}
            options={["Alle", ...PROJECTS]}
          />
          <Select
            label="Måling"
            value={measurement}
            onChange={setMeasurement}
            options={["Alle", ...MEASUREMENTS]}
          />
          <Select
            label="Status"
            value={status}
            onChange={setStatus}
            options={["Alle", "valid", "review", "blocked"]}
          />
        </div>
      </Card>
              ...
              {filtered.map((o, i) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 font-medium tabular-nums">
                    {o.value} <span className="text-xs text-muted-foreground">{o.unit}</span>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <DataQualityScore score={o.quality} />
                  </td>
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={9} className="px-4 py-8 text-center text-sm text-muted-foreground">
                    Ingen observationer matcher filteret.
                  </td>
                </tr>
              )}
            </tbody>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        ...
        <Section title="Måling over tid" subtitle="pH — SKB-WQ-01 sidste 24 t">
          <MiniLine
            values={[7.1, 7.2, 7.2, 7.3, 7.3, 7.4, 7.4, 7.4, 7.5, 7.4, 7.4, 7.3, 7.3, 7.4]}
          />
          <div className="mt-3 grid grid-cols-3 text-xs">
            ...
            <div>
              <div className="text-muted-foreground">Maks</div>
              <div className="font-medium">7,5</div>
            </div>
            <div>
              <div className="text-muted-foreground">Snit</div>
          ...
          <>
            ...
            <Section title="Routing">
              <div className="flex flex-wrap gap-1.5">
                {selected.routedTo.length === 0 ? (
                  <Chip>Ikke routet</Chip>
                ) : (
                  selected.routedTo.map((r: string) => (
                    <Chip key={r} tone="primary">
                      {r}
                    </Chip>
                  ))
            ...
            <Section title="Ledger event">
              <div className="text-xs font-mono bg-muted rounded-lg p-3">
                ledger://obs/{selected.source.toLowerCase()}/{selected.ts.replace(/:/g, "")}
              </div>
            </Section>
          </>
        )}
