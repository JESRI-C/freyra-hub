...
function NotificationsPage() {
  ...
  const setRule = (i: number, patch: Partial<AlertRule>) =>
  ...
  return (
    <main className="max-w-[1400px] mx-auto px-6 py-6 space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Administrér alarmer, rapporter og eskaleringer
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard
          label="Aktive regler"
          value={String(rules.filter((r) => r.email || r.inApp).length)}
          icon={<Bell className="h-5 w-5" />}
        />
        <StatCard
          label="E-mail aktiveret"
          value={String(rules.filter((r) => r.email).length)}
          icon={<Mail className="h-5 w-5" />}
        />
        <StatCard
          label="In-app aktiveret"
          value={String(rules.filter((r) => r.inApp).length)}
          icon={<MonitorSmartphone className="h-5 w-5" />}
        />
        <StatCard
          label="Høj-prioritet regler"
          value={String(rules.filter((r) => r.severity === "Høj").length)}
          icon={<Bell className="h-5 w-5" />}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Section title="Globale indstillinger" subtitle="Gælder for alle nye regler">
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <div className="text-sm">E-mail notifikationer</div>
              <Toggle checked={true} />
            </div>
            <div className="flex items-center justify-between">
              <div className="text-sm">In-app notifikationer</div>
              <Toggle checked={true} />
            </div>
            <div className="flex items-center justify-between">
              <div className="text-sm">Push på mobil</div>
              <Toggle checked={false} />
            </div>
            <div className="flex items-center justify-between">
              <div className="text-sm">Daglig sammenfatning</div>
              <Toggle checked={true} />
            </div>
            <div className="flex items-center justify-between">
              <div className="text-sm">Stille timer (22–07)</div>
              <Toggle checked={true} />
            </div>
          </div>
        </Section>
        ...
        <Section
          ...
          subtitle="Når en alarm ikke håndteres"
          ...
          <ol className="space-y-2">
            {[
              ...
              <li key={s.t} className="flex items-center gap-3 p-3 rounded-lg border">
                <div className="h-8 w-12 rounded-lg bg-leaf/20 text-primary grid place-items-center text-xs font-semibold">
                  {s.t === 0 ? "0 min" : s.t < 60 ? `${s.t}m` : `${s.t / 60}t`}
                </div>
              ...
              {rules.map((r, i) => (
                <tr key={r.key} className="border-t hover:bg-muted/20">
                  ...
                  <td className="px-4 py-2.5 whitespace-nowrap">
                    <span className="text-xs px-2 py-0.5 rounded bg-muted">{r.module}</span>
                  </td>
                  <td className="px-4 py-2.5">
                    <StatusPill status={r.severity} />
                  </td>
                  <td className="px-4 py-2.5">
                    <Toggle checked={r.email} onChange={(v) => setRule(i, { email: v })} />
                  </td>
                  <td className="px-4 py-2.5">
                    <Toggle checked={r.inApp} onChange={(v) => setRule(i, { inApp: v })} />
                  </td>
                  ...
                  <td className="px-4 py-2.5">
                    ...
                    >
                      <SettingsIcon className="h-3 w-3" /> Redigér
                    </button>
                  </td>
                </tr>
              ))}
      ...
      <Drawer
        ...
        title={open?.label ?? ""}
        subtitle={open ? `Modul: ${open.module}` : ""}
        footer={
          <>
            <button
        ...
        {open && openIdx !== null && (
          <>
            <Field
              label="Navn"
              value={open.label}
              onChange={(v) => setRule(openIdx, { label: v })}
            />
            <Select
            ...
            <Field
              ...
              value={open.recipients}
              onChange={(v) => setRule(openIdx, { recipients: v })}
            />
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-xl border p-3 flex items-center justify-between">
                <div>
                  <div className="text-xs text-muted-foreground">E-mail</div>
                  <div className="text-sm">Send via mail</div>
                </div>
                <Toggle checked={open.email} onChange={(v) => setRule(openIdx, { email: v })} />
              </div>
              <div className="rounded-xl border p-3 flex items-center justify-between">
                <div>
                  <div className="text-xs text-muted-foreground">In-app</div>
                  <div className="text-sm">Vis i platformen</div>
                </div>
                <Toggle checked={open.inApp} onChange={(v) => setRule(openIdx, { inApp: v })} />
              </div>
            </div>
            <div className="rounded-xl border bg-leaf/5 p-3 text-xs">
              Når denne regel udløses, registreres en hændelse i ESG Ledger og kan trigge eskalering
              jf. globale regler.
            </div>
          </>
        )}
