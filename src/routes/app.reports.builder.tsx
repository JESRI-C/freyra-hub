...
const GROUPS = ["Intro & resume", "ESG & compliance", "Natur & impact", "AI & beslutninger", "Data & verifikation", "Bilag"];
...
function Page() {
  ...
  const avgReadiness = ordered.length ? Math.round(ordered.reduce((s, x) => s + x.readiness, 0) / ordered.length) : 0;
  ...
  return (
    <main className="p-6 max-w-[1500px] w-full mx-auto space-y-4">
      <PageHeader title="Rapportbygger"
        description="Vælg afsnit, omarranger og styr indhold pr. afsnit. AI foreslår sektioner ud fra rapporttype."
        actions={
          <div className="flex gap-2">
            <button onClick={() => setSelected(recommended)} className="text-xs rounded-lg border bg-card px-3 py-1.5">Vælg alle anbefalede</button>
            <button onClick={() => setSelected((p) => p.filter((id) => !SECTION_LIBRARY.find((s) => s.id === id)?.missing))} className="text-xs rounded-lg border bg-card px-3 py-1.5"><Filter className="h-3 w-3 inline mr-1" /> Kun rapportklar data</button>
            <button className="text-xs rounded-lg bg-primary text-primary-foreground px-3 py-1.5"><Sparkles className="h-3 w-3 inline mr-1" /> Optimér til målgruppe</button>
          </div>
        }
      />
      ...
      <div className="grid lg:grid-cols-[280px_1fr_320px] gap-4">
        {/* LEFT: Library */}
        <Card className="p-3">
          <div className="text-xs font-semibold text-muted-foreground px-2 mb-2">Sektionsbibliotek</div>
            ...
            {GROUPS.map((g) => {
              ...
              return (
                <div key={g} className="rounded-lg border">
                  <button onClick={() => setOpenGroup(isOpen ? null : g)} className="w-full flex items-center justify-between px-2.5 py-2 text-sm font-medium">
                    {g}
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-4 gap-3">
            ...
            <Stat label="Ø klarhed" value={`${avgReadiness}%`} tone={avgReadiness >= 85 ? "success" : "warning"} />
          </div>

          <Section title="Rapportstruktur" subtitle={`${ordered.length} sektioner valgt · ${missing} med advarsler`}>
            {ordered.length === 0 ? (
              <div className="text-sm text-muted-foreground">Vælg sektioner i biblioteket til venstre.</div>
            ) : (
                    ...
                    <li key={s.id}>
                      <button onClick={() => setActiveId(s.id)}
                        ...
                        <div className="flex flex-col">
                          <button onClick={(e) => { e.stopPropagation(); move(s.id, -1); }} className="text-muted-foreground hover:text-foreground text-xs">▲</button>
                        ...
                        <div className="w-28"><ReadinessBar value={s.readiness} /></div>
                        <button onClick={(e) => { e.stopPropagation(); toggle(s.id); }} className="text-muted-foreground hover:text-destructive p-1"><Trash2 className="h-4 w-4" /></button>
                      </button>
                    </li>
                  );
                })}
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm mt-2 text-foreground/90">
                  For en <strong>investorrapport</strong> anbefales det at medtage <Chip tone="primary">Risikoanalyse</Chip>, <Chip tone="primary">Porteføljeimpact</Chip>, <Chip tone="primary">Datakvalitet</Chip> og <Chip tone="primary">Verifikationsstatus</Chip>. Disse er pt. ikke alle valgt.
                </p>
                <div className="mt-3 flex flex-wrap gap-2">
                  <button className="text-xs rounded-lg bg-primary text-primary-foreground px-3 py-1.5">Tilføj forslag</button>
                  <button className="text-xs rounded-lg border bg-card px-3 py-1.5">Medtag usikre data som bilag</button>
                  <button className="text-xs rounded-lg border bg-card px-3 py-1.5"><Send className="h-3 w-3 inline mr-1" /> Send mangler til DecisionsIQ</button>
                </div>
              </div>
            </div>
          </Card>
        ...
        <Card className="p-4 h-fit sticky top-[120px]">
          {active ? (
            <>
              ...
              <div className="mt-1 flex items-center gap-1.5"><Chip tone="muted">{active.module}</Chip><Chip>{active.group}</Chip></div>
              ...
              <div className="mt-4 space-y-2 text-sm">
                {[
                  ...
                  <label key={l} className="flex items-center justify-between py-1">
                    <span>{l}</span><input type="checkbox" defaultChecked={def} />
                  </label>
                ))}
              </div>
              ...
              <div className="mt-4">
                ...
                <select className="w-full rounded-lg border bg-card px-2.5 py-1.5 text-sm"><option>Standard</option><option>Kort</option><option>Dybdegående</option></select>
              </div>
              ...
              {active.missing && (
                <div className="mt-3 p-2.5 rounded-lg border bg-warning/10 text-xs">
                  <strong>Mangler:</strong> {active.missing}<br />
                  <span className="text-muted-foreground">Forslag: send opgave til Smart Connect for verifikation.</span>
                </div>
              )}

              <div className="mt-4 flex gap-2">
                <button className="flex-1 text-xs rounded-lg bg-primary text-primary-foreground py-2 inline-flex items-center justify-center gap-1.5"><Eye className="h-3.5 w-3.5" /> Preview sektion</button>
              </div>
            </>
          ) : (
...
function Stat({ label, value, tone }: { label: string; value: string; tone?: "success" | "warning" }) {
  const c = tone === "success" ? "text-success" : tone === "warning" ? "text-warning-foreground" : "";
  return (
    <Card className="p-3">
