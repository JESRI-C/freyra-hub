...
import { RISK_CATEGORIES, RISK_MATRIX_POINTS, RISK_TIMELINE, MITIGATIONS } from "@/lib/decisions-data";
...
function Page() {
  const overall = 58;
  const level = overall >= 75 ? "Kritisk" : overall >= 60 ? "Høj" : overall >= 40 ? "Medium" : "Lav";
  const c = 2 * Math.PI * 56;

  return (
    <main className="p-6 max-w-[1400px] w-full mx-auto space-y-5">
      <PageHeader title="Risikoanalyse" description="Samlet risikobillede på tværs af klima, natur, data og compliance." />
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        <Card className="p-6 flex flex-col items-center text-center">
          ...
          <div className="relative mt-4">
            <svg width="160" height="160" viewBox="0 0 140 140">
              ...
              <circle
                ...
                stroke={overall >= 60 ? "var(--destructive)" : overall >= 40 ? "var(--warning)" : "var(--success)"}
          ...
          <div className="mt-3"><RiskBadge level={level as any} /></div>
          <p className="mt-4 text-sm text-muted-foreground leading-relaxed">
            Risikoniveauet er <strong>medium</strong>. Datakvalitet og vandkvalitet er drivere; biodiversitet og compliance
            udvikler sig positivt. AI'en anbefaler fokus på de tre højest scorende risici.
          </p>
        </Card>

        <Card className="lg:col-span-2">
          <CardHeader title="Risikomatrix" subtitle="Sandsynlighed vs. konsekvens — hover for navn" />
      ...
      {/* Categories */}
      <div>
        <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-3">Risikokategorier</h3>
            ...
            return (
              <Card key={r.name} className="p-5">
                <div className="flex items-start justify-between">
                  <div className="h-9 w-9 rounded-xl bg-destructive/10 text-destructive grid place-items-center"><ShieldAlert className="h-4 w-4" /></div>
                ...
                <div className="mt-2 flex items-baseline gap-2">
                  ...
                  <div className={`text-xs inline-flex items-center gap-0.5 ${down ? "text-success" : "text-destructive"}`}>
      ...
      <div className="grid lg:grid-cols-2 gap-5">
        ...
        <Card>
          <CardHeader title="Mitigation-plan" subtitle="Anbefalede handlinger med forventet effekt" />
          <table className="w-full text-sm">
            <thead className="text-left text-xs text-muted-foreground border-y bg-muted/40">
              <tr><th className="px-5 py-2">Handling</th><th className="py-2">Ejer</th><th className="py-2">Deadline</th><th className="py-2">Effekt</th><th className="py-2">Status</th></tr>
            </thead>
            <tbody className="divide-y">
              {MITIGATIONS.map((m, i) => (
                <tr key={i}>
                  ...
                  <td><Pill tone={m.status === "Igangsat" ? "info" : m.status === "Planlagt" ? "warning" : "default"}>{m.status}</Pill></td>
                </tr>
              ))}
            </tbody>
          </table>
