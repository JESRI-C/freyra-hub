    ...
    <main className="max-w-[1400px] mx-auto px-6 py-6 space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Plan, forbrug og fremtidig fakturering</p>
        </div>
      </div>
      ...
      <div className="rounded-xl border bg-leaf/5 p-3 text-xs flex gap-2 items-start">
        ...
        <div>
          Betaling og fakturering aktiveres senere. Denne side viser den planlagte
          abonnementsstruktur og dit aktuelle forbrug.
        </div>
      </div>
        ...
        <Section title="Faktureringskontakt">
          <div className="space-y-2 text-sm">
            <div className="flex items-center gap-2">
              <Mail className="h-3.5 w-3.5 text-muted-foreground" /> {PLAN.contact}
            </div>
            <div className="flex items-center gap-2">
              <FileText className="h-3.5 w-3.5 text-muted-foreground" /> Faktura sendes som PDF
            </div>
            <div className="flex items-center gap-2">
              <CreditCard className="h-3.5 w-3.5 text-muted-foreground" /> Aktiveres senere — ingen
              kortdata påkrævet
            </div>
          </div>
          <div className="mt-3 flex gap-2">
            <button className="text-xs rounded-lg border bg-card px-3 py-1.5">Skift kontakt</button>
            <button className="text-xs rounded-lg border bg-card px-3 py-1.5">Hent kontrakt</button>
          </div>
        </Section>
      </div>
      ...
      <Section title="Tilgængelige planer" subtitle="Sammenlign og vælg den rigtige plan">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {PLANS.map((p) => (
            <div
              key={p.name}
              className={`rounded-2xl border p-5 flex flex-col ${p.current ? "border-primary bg-leaf/5 shadow-soft" : "bg-card"}`}
            >
              ...
              <button
                className={`mt-4 rounded-xl px-3 py-1.5 text-sm ${p.current ? "border bg-card" : "bg-primary text-primary-foreground"}`}
                disabled={p.current}
              >
            ...
            {[
              ...
              <div key={a.n} className="rounded-xl border p-3 flex flex-col">
                ...
                <div className="mt-2 flex items-center justify-between">
                  <span className="text-xs font-medium">{a.p}</span>
                  <button className="text-xs rounded-lg border bg-card px-2 py-1">Tilføj</button>
                </div>
              </div>
            ))}
          </div>
