  ...
  return (
    <main className="p-6 max-w-[1400px] w-full mx-auto space-y-5">
      <div className="grid sm:grid-cols-2 xl:grid-cols-5 gap-3">
        ...
        <ESGMetricCard
          label="Verifikationsnoter"
          value="9"
          icon={<ShieldCheck className="h-4 w-4" />}
          tone="success"
        />
        <ESGMetricCard
          label="Mangler godkendelse"
          value="7"
      ...
      <Card className="overflow-hidden">
        <CardHeader
          ...
          subtitle="Klik for forhåndsvisning og versionshistorik"
        />
      ...
      {open && (
        ...
        >
          <div
            className="w-full max-w-2xl h-full bg-card border-l shadow-card overflow-y-auto"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="p-5 border-b flex items-start justify-between">
              <div className="min-w-0">
                <div className="text-xs text-muted-foreground">
                  {open.type} · {open.version}
                </div>
                <div className="font-semibold text-lg truncate">{open.name}</div>
                <div className="text-xs text-muted-foreground mt-0.5">
                  {open.project} · ansvarlig: {open.owner}
                </div>
              </div>
              <button
                onClick={() => setOpenId(null)}
            ...
            <div className="p-6 space-y-5 text-sm">
              ...
              <div className="rounded-xl border bg-background p-5">
                ...
                <Section title="Resumé">
                  Dokumentet samler nøgleresultater og dokumentation for {open.metric.toLowerCase()}{" "}
                  i {open.project}. Indholdet er sporbart til kilder i ESG Ledger og er klar til
                ...
                <Section title="Kildedata">
                  Sensorer, Sentinel-2, ERP, Impact Exchange og tredjepartsverifikation.
                  Krydsvalideret med baseline og kontrolleret for fuldstændighed og konsistens.
                </Section>
                <Section title="Verifikationsstatus">
                  Tredjepartsreview {open.status === "Godkendt" ? "fuldført" : "i gang"}.
                  Audit-trail vedhæftet som bilag.
                </Section>
              </div>
              ...
              <div>
                ...
                <ul className="rounded-xl border divide-y text-xs">
                  <li className="px-3 py-2 flex justify-between">
                    <span>
                      {open.version} · {open.created}
                    </span>
                    <span className="text-muted-foreground">Aktuel</span>
                  </li>
                  <li className="px-3 py-2 flex justify-between">
                    <span>v0.9 · 2026-04-22</span>
                    <span className="text-muted-foreground">Intern review afsluttet</span>
                  </li>
                  <li className="px-3 py-2 flex justify-between">
                    <span>v0.5 · 2026-04-10</span>
                    <span className="text-muted-foreground">Udkast oprettet</span>
                  </li>
                </ul>
              </div>
              ...
              <div className="flex flex-wrap gap-2 justify-end pt-2">
                <button className="text-sm rounded-lg border px-3 py-2 hover:bg-muted inline-flex items-center gap-1.5">
                  <Download className="h-4 w-4" /> Eksportér PDF
                </button>
                <button className="text-sm rounded-lg border px-3 py-2 hover:bg-muted inline-flex items-center gap-1.5">
                  <Send className="h-4 w-4" /> Send til rapportering
                </button>
                <button className="text-sm rounded-lg bg-primary text-primary-foreground px-3 py-2 inline-flex items-center gap-1.5">
                  <Send className="h-4 w-4" /> Send til revisorpakke
                </button>
              </div>
            </div>
          </div>
