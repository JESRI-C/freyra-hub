...
const ACTIVE = [
  ...
  },
  {
    name: "Sentinel Hub",
    type: "Satellite",
    project: "Urban Biodiversity Corridor",
    status: "Aktiv",
    lastSync: "6 t",
    auth: "OAuth 2.0",
    volume: "12 layers",
...
function Page() {
  ...
  return (
    <main className="p-6 max-w-[1400px] w-full mx-auto space-y-4">
      <PageHeader
        title="Integrationer"
        description="Eksterne systemer, protokoller og API-forbindelser."
      />
            ...
            <tbody className="divide-y">
              {ACTIVE.map((a) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-xs">
                    {a.errors > 0 ? <span className="text-destructive">{a.errors}</span> : "0"}
                  </td>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-foreground/90 mt-2">
                Smart Connect gemmer adgangsdata sikkert og viser kun forbindelsesstatus, mapping og
                audit-events i brugerfladen. Hemmelige nøgler, tokens og certifikater er aldrig
                synlige for brugere.
              </p>
            </div>
          </div>
        ...
        footer={
          <>
            ...
            <button className="text-xs rounded-lg border bg-card px-3 py-1.5 inline-flex items-center gap-1.5">
              <KeyRound className="h-3.5 w-3.5" /> Opdater credentials
            </button>
            <button className="text-xs rounded-lg border bg-card px-3 py-1.5 inline-flex items-center gap-1.5">
              <FileText className="h-3.5 w-3.5" /> Se logs
            </button>
            <button className="text-xs rounded-lg border bg-card px-3 py-1.5 inline-flex items-center gap-1.5">
        ...
        {selected && (
          <>
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <KV
                label="Fejl"
                v={selected.errors > 0 ? `${selected.errors} sidste 24 t` : "Ingen"}
              />
            </div>
            ...
            <Section title="Sikkerhedsstatus">
              <ul className="text-xs space-y-1">
                <li className="flex items-center gap-2">
                  <CheckCircle2 className="h-3.5 w-3.5 text-success" /> TLS 1.3
                </li>
                <li className="flex items-center gap-2">
                  <CheckCircle2 className="h-3.5 w-3.5 text-success" /> Token rotation aktiv
                </li>
                <li className="flex items-center gap-2">
                  <CheckCircle2 className="h-3.5 w-3.5 text-success" /> Audit log aktiveret
                </li>
              </ul>
            </Section>
          </>
