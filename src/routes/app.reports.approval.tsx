...
const REVIEWERS = [
  ...
  },
  {
    ...
    role: "Data Manager",
    status: "Afventer",
    when: "Tildelt 2 dage siden",
    initials: "MH",
  },
  {
    name: "Eksternt review (DNV)",
    role: "Verifier",
    status: "Ikke startet",
    when: "Valgfri",
    initials: "DN",
...
const COMMENTS = [
  {
    who: "Jesper Riel",
    when: "I går",
    text: "Forkort executive summary til 2 afsnit. Ellers ser det stærkt ud.",
    resolved: true,
  },
  {
    who: "Mikkel Holm",
    when: "2 dage siden",
    text: "Kan vi tilføje datakvalitets-score per kilde i bilaget?",
    resolved: true,
  },
  {
    who: "Emma Larsen",
    when: "3 dage siden",
    text: "Verificér at biodiversitetsindeks bruger Q2-baseline, ikke Q1.",
    resolved: false,
  },
];
...
const EXPORT_OPTIONS = [
  {
    ...
    icon: FileText,
    name: "PDF",
    best: "Ekstern deling, kunder, investorer",
    includes: "Forside, alle valgte sektioner, bilag, audit trail-reference",
    status: "Klar",
  ...
  },
  {
    ...
    icon: FileType2,
    name: "Word",
    best: "Redigerbar version til samarbejde",
    includes: "Tekst, tabeller, billeder — bevarer struktur",
    status: "Klar",
    tone: "success" as const,
  },
  {
    ...
    name: "Excel",
    best: "Datatabeller og rådata",
    includes: "ESG-metrics, CO₂-regnskab, datakilder, emissionsfaktorer",
    status: "Klar",
    tone: "success" as const,
  },
  {
    ...
    icon: FileSpreadsheet,
    name: "CSV",
    best: "Maskinlæsbare data til import",
    includes: "Flade datatabeller fra ESG Ledger",
    status: "Klar",
    tone: "success" as const,
  },
  {
    ...
    icon: Link2,
    name: "Delbart link",
    best: "Hurtig deling med adgangskontrol",
    includes: "Skrivebeskyttet preview, valgfri udløbsdato",
    status: "Klar",
    tone: "success" as const,
  },
  {
    ...
    icon: ShieldCheck,
    name: "Revisorpakke",
    best: "Ekstern revision og verifikation",
    includes: "Rapport + audit trail + emissionsfaktorer + dokumenter",
    status: "Kræver godkendelse",
    tone: "warning" as const,
  },
  {
    key: "ledger",
    icon: Archive,
    name: "ESG Ledger archive",
    best: "Permanent, immutable arkivering",
    includes: "Versioneret rapport + hash i ledger",
    status: "Klar",
    tone: "success" as const,
  },
  {
    ...
    icon: Briefcase,
    name: "Board pack",
    best: "Bestyrelsesmøder",
    includes: "Forkortet ledelsesresumé + kerne-KPIs + risici",
    status: "Klar",
    tone: "success" as const,
  },
];
...
function ApprovalPage() {
  ...
  return (
    <main className="max-w-[1400px] mx-auto px-6 py-6 space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Skallebæk Naturimpact Q2 2026 · v1.4 · Kunde
            </p>
          </div>
        </div>
      ...
      <Section
        title="Godkendelsesflow"
        subtitle="Klik på et trin for at se detaljer"
        action={
          <Chip tone="primary">
            Trin {current + 1} af {STEPS.length}
          </Chip>
        }
      >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-2">
            {REVIEWERS.map((r) => (
              <div key={r.name} className="flex items-center gap-3 p-2.5 rounded-lg border">
                <div className="h-9 w-9 rounded-full bg-leaf/30 text-primary grid place-items-center text-xs font-semibold">
                  {r.initials}
                </div>
        ...
        <Section
          title="Kommentarer"
          subtitle="Review-feedback og afklaringer"
          action={
            <button className="text-xs rounded-lg border bg-card px-2.5 py-1 inline-flex items-center gap-1">
        ...
        >
          <div className="space-y-2">
            {COMMENTS.map((c, i) => (
              <div
                key={i}
                className={`rounded-lg border p-3 ${c.resolved ? "bg-success/5" : "bg-warning/5"}`}
              >
                <div className="flex items-center justify-between gap-2">
                  <div className="text-xs font-medium">{c.who}</div>
                  <div className="flex items-center gap-2">
                    <Chip tone={c.resolved ? "success" : "warning"}>
                      {c.resolved ? "Løst" : "Åben"}
                    </Chip>
        ...
        <Section title="Godkendelses-checkliste" subtitle="Skal være opfyldt før ekstern eksport">
          <ul className="space-y-1.5">
            {checks.map((c, i) => (
              <li key={c.label}>
                <button
                  onClick={() =>
                    setChecks((prev) => prev.map((x, j) => (j === i ? { ...x, done: !x.done } : x)))
                  }
                  className="w-full flex items-center gap-3 text-left px-2 py-1.5 rounded-lg hover:bg-muted"
                >
                  <span
                    className={`h-5 w-5 rounded-md border grid place-items-center flex-shrink-0 ${c.done ? "bg-success border-success text-white" : "bg-card"}`}
                  >
                    {c.done && <CheckCircle2 className="h-3.5 w-3.5" />}
                  </span>
                  <span className={`text-sm flex-1 ${c.done ? "" : "text-foreground"}`}>
                    {c.label}
                  </span>
                </button>
              </li>
          ...
          <div
            className={`mt-3 rounded-lg p-2.5 text-xs ${allOk ? "bg-success/10 text-success" : "bg-warning/10 text-warning-foreground"}`}
          >
            {allOk
              ? "Alle krav opfyldt — klar til endelig eksport."
              : "Mangler godkendelse på ledelses- og ekstern brug."}
          </div>
        </Section>
      </div>
            ...
            return (
              <div key={e.key} className="rounded-xl border bg-card p-4 flex flex-col">
                <div className="flex items-center justify-between">
                  <div className="h-9 w-9 rounded-xl grid place-items-center bg-leaf/30 text-primary">
                    <Icon className="h-4 w-4" />
                  </div>
                  <Chip tone={isExported ? "success" : e.tone}>
                    {isExported ? "Eksporteret" : e.status}
                  </Chip>
                </div>
                ...
                >
                  {isExported ? (
                    <>
                      <CheckCircle2 className="h-3.5 w-3.5" /> Klar til download
                    </>
                  ) : (
                    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="grid grid-cols-1 gap-2">
            {[
              ...
              return (
                <button
                  key={a.label}
                  className="flex items-center gap-3 px-3 py-2 rounded-lg border bg-card hover:bg-muted text-sm"
                >
        ...
        <Section title="AI-anbefaling" subtitle="DecisionsIQ foreslår">
          <div className="rounded-xl border bg-leaf/10 p-4 text-sm">
            <p>
              Rapporten er klar til intern brug og kunde-deling, men{" "}
              <span className="font-medium">
                ekstern revisor-brug bør afvente feltverifikation i Zone C
              </span>
              . Generér også en kort 2-siders <span className="font-medium">Projektfakta</span>
              -version til sales og partner-udsendelse.
            </p>
            <div className="mt-3 flex gap-2 flex-wrap">
              <button className="text-xs rounded-lg bg-primary text-primary-foreground px-2.5 py-1">
                Generér Projektfakta
              </button>
              <button className="text-xs rounded-lg border bg-card px-2.5 py-1">
                Send til feltteam
              </button>
            </div>
          </div>
