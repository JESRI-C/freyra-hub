...
import { Building2, Users, ShieldCheck, Landmark, Heart, Briefcase, X, ArrowRight } from "lucide-react";
  ...
  return (
    <main className="p-6 max-w-[1400px] w-full mx-auto space-y-5">
      <div className="grid sm:grid-cols-3 xl:grid-cols-6 gap-3">
        <ImpactMetricCard label="Projektudbydere" value={`${count("Projektudbyder")}`} icon={<Briefcase className="h-4 w-4" />} />
        <ImpactMetricCard label="Købere/investorer" value={`${count("Køber/investor") || 4}`} icon={<Users className="h-4 w-4" />} />
        <ImpactMetricCard label="Verifikationspartnere" value={`${count("Verifikationspartner") || 3}`} icon={<ShieldCheck className="h-4 w-4" />} />
        <ImpactMetricCard label="Kommuner" value={`${count("Kommune")}`} icon={<Landmark className="h-4 w-4" />} />
        <ImpactMetricCard label="NGO'er" value={`${count("NGO") || 2}`} icon={<Heart className="h-4 w-4" />} />
        <ImpactMetricCard label="Virksomheder" value={`${count("Virksomhed") || 5}`} icon={<Building2 className="h-4 w-4" />} />
      </div>

      <Card className="overflow-hidden">
        <CardHeader title="Organisationer i økosystemet" subtitle="Klik på en organisation for detaljer" />
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y">
            {ORGANIZATIONS.map((o) => {
              ...
              return (
                <tr key={o.id} className="hover:bg-muted/30 cursor-pointer" onClick={() => setOpenId(o.id)}>
                  <td className="px-5 py-3">
                    <div className="inline-flex items-center gap-2 font-medium"><Icon className="h-4 w-4 text-primary" /> {o.name}</div>
                  </td>
                  ...
                  <td><Pill tone={o.contactStatus === "Aktiv" ? "success" : "warning"}>{o.contactStatus}</Pill></td>
                  <td>
                    <div className="flex items-center gap-2">
                      <div className="h-1.5 w-16 rounded-full bg-muted overflow-hidden">
                        <div className={`h-full ${o.trust >= 90 ? "bg-success" : "bg-leaf"}`} style={{ width: `${o.trust}%` }} />
                      </div>
                      <span className="text-xs tabular-nums">{o.trust}</span>
                    </div>
                  </td>
                  <td className="pr-5"><button className="text-xs text-primary hover:underline">Se profil</button></td>
                </tr>
              );
            })}
          </tbody>
            ...
            <div className="p-5 space-y-4 text-sm">
              ...
              <div>
                ...
                <ul className="rounded-xl border divide-y text-xs">
                  <li className="px-3 py-2 flex justify-between"><span>Seneste audit</span><span className="text-muted-foreground">2026-04-12</span></li>
                  <li className="px-3 py-2 flex justify-between"><span>Antal verifikationer</span><span className="text-muted-foreground">{open.projects * 2}</span></li>
                  <li className="px-3 py-2 flex justify-between"><span>Standardefterlevelse</span><span className="text-success">100%</span></li>
                </ul>
              </div>
              <div>
              ...
              <div className="flex gap-2 pt-2">
                <button className="flex-1 text-sm rounded-lg border px-3 py-2 hover:bg-muted">Dokumenter</button>
                <button className="flex-1 text-sm rounded-lg bg-primary text-primary-foreground px-3 py-2">Kontakt</button>
              </div>
            </div>
          </div>
        </div>
