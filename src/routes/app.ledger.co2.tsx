...
function CO2Page() {
  const scope1 = EMISSION_SOURCES.filter((e) => e.scope === 1).reduce(
    (s, e) => s + Math.max(e.co2e, 0),
    0,
  );
  const scope2 = EMISSION_SOURCES.filter((e) => e.scope === 2).reduce(
    (s, e) => s + Math.max(e.co2e, 0),
    0,
  );
  const scope3 = EMISSION_SOURCES.filter((e) => e.scope === 3).reduce(
    (s, e) => s + Math.max(e.co2e, 0),
    0,
  );
  const offsets = Math.abs(
    EMISSION_SOURCES.filter((e) => e.co2e < 0).reduce((s, e) => s + e.co2e, 0),
  );
    ...
    <main className="p-6 max-w-[1400px] w-full mx-auto space-y-5">
      <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-4">
        <ESGMetricCard
          ...
          hint="Efter dokumenterede reduktioner"
        />
        <ESGMetricCard
          label="Scope 1"
          value={scope1.toLocaleString("da-DK")}
          unit="t"
          trend={-5}
          icon={<Cloud className="h-4 w-4" />}
        />
        <ESGMetricCard
          label="Scope 2"
          value={scope2.toLocaleString("da-DK")}
          unit="t"
          trend={-12}
          icon={<Cloud className="h-4 w-4" />}
        />
        <ESGMetricCard
          label="Scope 3"
          value={scope3.toLocaleString("da-DK")}
          unit="t"
          trend={-3}
          icon={<Cloud className="h-4 w-4" />}
          tone="warning"
          ...
          <div className="px-5 pb-5">
            <LineChart
              series={[
                {
                  label: "Baseline-bane",
                  values: [60, 58, 55, 52, 49, 46],
                  color: "oklch(0.7 0.02 160)",
                },
                {
                  label: "Mål-bane",
                  values: [60, 56, 51, 46, 41, 36],
            ...
            <div className="flex flex-wrap gap-4 text-xs text-muted-foreground mt-2">
              <span className="inline-flex items-center gap-1.5">
                <span
                  ...
                  style={{ background: "oklch(0.7 0.02 160)" }}
                />
                Baseline 2020
              </span>
              <span className="inline-flex items-center gap-1.5">
          ...
          <tbody className="divide-y">
            {EMISSION_SOURCES.map((e) => (
              <tr key={e.id} className={e.co2e < 0 ? "bg-success/5" : ""}>
                ...
                <td
                  className={`text-right tabular-nums font-medium ${e.co2e < 0 ? "text-success" : ""}`}
                >
                  {e.co2e.toLocaleString("da-DK")}
                </td>
                <td className="text-xs">{e.method}</td>
                <td className="pr-3">
                  <ReadinessScore label="" value={e.quality} size="sm" />
                </td>
                <td>
                  <VerificationStatusBadge status={e.verification} />
                </td>
                <td>
                  <Pill
                    tone={
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        ...
        <Card className="lg:col-span-2">
          <CardHeader
            title="Emissionsfaktorer · gennemgang"
            subtitle="Kilder, gyldighed og review-status"
          />
              ...
              <li key={r.f} className="py-2.5 flex items-center gap-3">
                <div className="flex-1">
                  ...
                  <div className="text-xs text-muted-foreground">
                    Kilde: {r.source} · sidst gennemgået {r.reviewed}
                  </div>
                </div>
                <div className="text-xs tabular-nums w-12 text-right">{r.conf}%</div>
                {r.needs ? <Pill tone="warning">Validér</Pill> : <Pill tone="success">OK</Pill>}
              </li>
            ))}
          </ul>
        </Card>
        ...
        <Card>
          ...
          <div className="px-5 pb-5 grid gap-2">
            <ActionBtn
              icon={<ClipboardCheck className="h-4 w-4" />}
              label="Validér emissionsfaktorer"
            />
            ...
            <ActionBtn
              ...
              label="Markér datakilde som verificeret"
            />
