  ...
  default: [
    {
      ...
      when: "I dag · 09:42",
      changes: [
        "Executive summary opdateret",
        "Biodiversitetsindeks Q2 indlæst",
        "AI-anbefalinger genereret",
      ],
      comment: "Klar til intern review",
      exported: false,
      approved: false,
    },
    {
      ...
      by: "Mikkel Holm",
      when: "I går · 16:08",
      changes: ["Datakilder valideret", "Audit trail synkroniseret"],
      comment: "Smart Connect data refreshed",
      exported: false,
      approved: false,
    },
    {
      ...
      by: "Emma Larsen",
      when: "3 dage siden",
      changes: ["Vandkvalitet-graf tilføjet", "Metodebilag opdateret"],
      comment: "Kunde-feedback indarbejdet",
      exported: true,
      approved: true,
    },
    {
      ...
      by: "Jesper Riel",
      when: "1 uge siden",
      changes: ["Forside og branding tilrettet"],
    ...
    },
    {
      ...
      by: "Emma Larsen",
      when: "2 uger siden",
      changes: ["Første udkast genereret"],
      comment: "AI-udkast",
      exported: false,
      approved: false,
    },
...
const ACTIVITY = [
  {
    icon: FileEdit,
    label: "Kladde oprettet fra skabelon Naturimpact-rapport",
    who: "Emma Larsen",
    when: "2 uger siden",
  ...
  {
    icon: FileText,
    label: "Sektion 'Biodiversitetsindeks' tilføjet",
    who: "Emma Larsen",
    when: "12 dage siden",
    tone: "default" as const,
  },
  {
    icon: Download,
    label: "Smart Connect data refresh — Skallebæk Zone A/B",
    who: "System",
    when: "8 dage siden",
    tone: "info" as const,
  },
  {
    icon: ShieldCheck,
    label: "AI-summary opdateret af DecisionsIQ",
    who: "AI",
    when: "5 dage siden",
    tone: "info" as const,
  },
  ...
  {
    icon: MessageSquare,
    label: "Kommentar fra Jesper Riel: 'Forkort exec summary'",
    who: "Jesper Riel",
    when: "3 dage siden",
    tone: "default" as const,
  },
  ...
  {
    icon: Download,
    label: "PDF eksporteret (v1.2) — 18 sider",
    who: "Emma Larsen",
    when: "I dag · 09:00",
    tone: "success" as const,
  },
  {
    icon: Archive,
    label: "Arkivkopi gemt i ESG Ledger (LDG-2041)",
    who: "System",
    when: "I dag · 09:01",
    tone: "success" as const,
  },
...
function LibraryPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (filter === "mine") r = r.filter((x) => x.owner === "Emma Larsen");
      ...
      r = r.filter((x) => x.status === "Kræver data" || x.readiness < 80);
    else if (filter === "archived") r = r.filter((x) => x.status === "Arkiveret");
    if (q)
      r = r.filter((x) =>
        (x.name + x.type + x.scope + x.owner).toLowerCase().includes(q.toLowerCase()),
      );
    return r;
  }, [filter, q]);
  ...
  return (
    <main className="max-w-[1400px] mx-auto px-6 py-6 space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Alle gemte rapporter, versioner og eksporthistorik
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        />
        <StatCard
          label="Afventer review"
          value="5"
          icon={<Clock className="h-5 w-5" />}
          accent="bg-leaf/30 text-primary"
        />
        <StatCard
          label="Kladder"
          value="6"
      ...
      <Card className="overflow-hidden">
        <div className="flex items-center justify-between p-4 border-b gap-3">
          <div className="text-sm font-semibold">
            Rapporter <span className="text-muted-foreground font-normal">({filtered.length})</span>
          </div>
          <div className="flex items-center gap-2 rounded-lg border bg-card px-2.5 py-1.5 w-72 max-w-full">
            ...
            <input
              ...
              placeholder="Søg rapport, projekt eller ejer..."
              ...
              {filtered.map((r) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 whitespace-nowrap">
                    <Chip tone="muted">{r.version}</Chip>
                  </td>
                  <td className="px-4 py-3">
                    <ReportStatusBadge status={r.status} />
                  </td>
                  <td className="px-4 py-3">
                    <ReadinessScore value={r.readiness} size="sm" />
                  </td>
                  ...
                  <td className="px-4 py-3 whitespace-nowrap">
                    <div className="flex gap-1">
                      <button
                        ...
                        className="text-xs rounded-lg border px-2 py-1 hover:bg-muted inline-flex items-center gap-1"
                      >
                        <History className="h-3 w-3" /> Versioner
                      </button>
                      <button
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={11} className="px-4 py-12 text-center text-sm text-muted-foreground">
                    Ingen rapporter matcher dette filter.
                  </td>
                </tr>
              )}
            </tbody>
      ...
      <Drawer
        ...
        title={open?.name ?? ""}
        subtitle={open ? `${open.id} · ${open.type} · ${open.scope}` : ""}
        width="max-w-2xl"
        footer={
          <>
            <button className="rounded-lg border bg-card px-3 py-1.5 text-sm">Åbn i bygger</button>
            <button className="rounded-lg border bg-card px-3 py-1.5 text-sm inline-flex items-center gap-1.5">
              <Download className="h-3.5 w-3.5" /> Eksportér seneste version
            </button>
          ...
          <>
            <div className="grid grid-cols-3 gap-3">
              <div className="rounded-xl border p-3">
                ...
                <div className="mt-1">
                  <ReportStatusBadge status={open.status} />
                </div>
              </div>
              <div className="rounded-xl border p-3">
                <div className="text-[11px] text-muted-foreground">Readiness</div>
                <div className="mt-1">
                  <ReadinessScore value={open.readiness} size="sm" />
                </div>
              </div>
              <div className="rounded-xl border p-3">
                <div className="text-[11px] text-muted-foreground">Ejer</div>
                <div className="mt-1 text-sm">{open.owner}</div>
              </div>
            </div>
              ...
              <ol className="space-y-2">
                {ACTIVITY.map((a, i) => {
                  ...
                  const tones: Record<string, string> = {
                    ...
                    info: "bg-leaf/30 text-primary",
                    success: "bg-success/15 text-success",
                  };
                  return (
                    <li key={i} className="flex gap-3 items-start">
                      <div
                        className={`h-7 w-7 rounded-lg grid place-items-center flex-shrink-0 ${tones[a.tone]}`}
                      >
